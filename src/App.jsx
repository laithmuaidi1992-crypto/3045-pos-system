...
<img src={STORE_LOGO} alt="3045" style={{height:paperSize==="A4"?100:paperSize==="80mm"?70:55,marginBottom:8}}/>
...
try{
  ...
  if(setupData.wipeTest){
    try{
      ...
      await sb.from("transaction_items").delete().neq("id",0);
      await sb.from("transactions").delete().neq("id",0);
      await sb.from("sales_returns").delete().neq("id",0);
      await sb.from("purchase_returns").delete().neq("id",0);
      await sb.from("purchase_invoice_items").delete().neq("id",0);
      await sb.from("purchase_invoices").delete().neq("id",0);
      await sb.from("expenses").delete().neq("id",0);
      await sb.from("money_movements").delete().neq("id",0);
      await sb.from("cash_shifts").delete().neq("id",0);
      await sb.from("eod_reports").delete().neq("id",0);
      await sb.from("attendance").delete().neq("id",0);
      await sb.from("loyalty_transactions").delete().neq("id",0);
      await sb.from("coupon_redemptions").delete().neq("id",0);
      await sb.from("salary_payments").delete().neq("id",0);
      try{await sb.from("stocktake_items").delete().neq("id",0);await sb.from("stocktake_sessions").delete().neq("id",0)}catch{}
      // Reset all product stock to 0
      await sb.from("products").update({stock:0}).neq("id",0);
      ...
      const np=await DB.getProducts();setProds(np);
    }catch(e){console.error("Wipe error:",e);sT("⚠ "+(rtl?"خطأ في التنظيف":"Cleanup error"),"err")}
  }
...
}catch(e){console.error(e);sT("✗ "+(rtl?"خطأ":"Error"),"err")}
