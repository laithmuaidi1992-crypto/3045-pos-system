...
const DB = {
  ...
  async addTransaction(tx, cashierId, cashierName) {
    ...
    // Decrease stock (allows negative values for tracking oversold items)
...
const[stocktakeItems,setStocktakeItems]=useState({});const[stocktakeStarted,setStocktakeStarted]=useState(false);
...
const addToCart=useCallback(p=>{if(p.u==="kg"||p.u==="g"){const w=prompt(rtl?"أدخل الوزن (كغ):":"Enter weight (kg):","1");if(!w)return;const wt=parseFloat(w);if(isNaN(wt)||wt<=0)return;beep();speak((p.a||p.n)+" "+wt);setCart(prev=>[...prev,{...p,id:p.id+"_w"+Date.now(),qty:1,p:+(p.p*wt).toFixed(3),_weight:wt,_origPrice:p.p}]);return;}beep();speak(p.a||p.n);try{const pBatches=batches.filter(b=>b.product_id===p.id&&b.status==="active"&&b.quantity_remaining>0);if(pBatches.length>0){const nearest=pBatches.sort((a,b)=>new Date(a.expiry_date)-new Date(b.expiry_date))[0];if(nearest&&nearest.expiry_date){const dLeft=Math.ceil((new Date(nearest.expiry_date)-new Date())/86400000);if(dLeft<=7&&dLeft>0)sT("⚠️ "+(rtl?"قارب على الانتهاء — ":"Near expiry — ")+dLeft+(rtl?" أيام":" days left"),"err")}}}catch(e){}setCart(prev=>{const ex=prev.find(i=>i.id===p.id);return ex?prev.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i):[...prev,{...p,qty:1}]})},[beep,voiceOn,batches,rtl]);
    ...
    for(const item of cart){
      ...
      if(pkg){
        ...
        try{
          ...
          if(parent){await sb.from("products").update({stock:parent.stock-deductQty}).eq("id",pkg.parentId)}
...
<td><button className="ab ab-d" onClick={async()=>{setExpensesList(p=>p.filter(x=>x.id!==e.id));try{await DB.deleteExpense(e.id);const attachments={};delete attachments[e.id];/* attachments disabled */}catch{}}}>✕</button></td>
</tr>})}</tbody></table>}
</>}
...
<div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
<button onClick={()=>{const csv=["Barcode,Product,System,Counted,Variance"];prods.forEach(p=>{const c=stocktakeItems[p.id];if(c!==undefined){const v=parseInt(c)-p.s;csv.push('"'+p.bc+'","'+pN(p)+'",'+p.s+","+c+","+v)}});const b=new Blob([csv.join("\n")],{type:"text/csv"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="stocktake-"+new Date().toISOString().slice(0,10)+".csv";a.click()}} style={{padding:"8px 16px",background:"#2563eb",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>📥 {rtl?"تصدير":"Export CSV"}</button>
<button onClick={async()=>{if(!confirm(rtl?"تطبيق التعديلات على المخزون؟":"Apply adjustments to stock?"))return;let n=0;for(const p of prods){const c=stocktakeItems[p.id];if(c!==undefined){const ns=parseInt(c)||0;if(ns!==p.s){try{await DB.updateProductPriceStock(p.id,p.p,ns,p.exp);n++}catch(e){}}}}const np=await DB.getProducts();setProds(np);sT("✓ "+(rtl?"تم تعديل":"Adjusted ")+n+(rtl?" منتج":" products"),"ok")}} style={{padding:"8px 16px",background:"#059669",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>✓ {rtl?"تطبيق التعديلات":"Apply Adjustments"}</button>
<button onClick={()=>{if(confirm(rtl?"إلغاء الجرد؟":"Cancel stocktake?")){setStocktakeStarted(false);setStocktakeItems({})}}} style={{padding:"8px 16px",background:"#fff",border:"1.5px solid #dc2626",borderRadius:8,color:"#dc2626",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>✕ {rtl?"إلغاء":"Cancel"}</button>
<div style={{marginLeft:"auto",fontSize:12,color:"#6b7280"}}>{Object.keys(stocktakeItems).length} / {prods.length} {rtl?"معدود":"counted"}</div>
</div>
<div style={{overflowX:"auto"}}>
<table className="at"><thead><tr><th>{t.bc}</th><th>{t.product}</th><th>{rtl?"النظام":"System"}</th><th>{rtl?"المعدود":"Counted"}</th><th>{rtl?"الفرق":"Variance"}</th></tr></thead>
<tbody>{prods.map(p=>{const c=stocktakeItems[p.id];const v=c!==undefined?(parseInt(c)||0)-p.s:null;return<tr key={p.id} style={{background:v===null?"transparent":v===0?"#ecfdf5":v>0?"#eff6ff":"#fef2f2"}}>
...
{["58mm","80mm","A4"].map(sz=><button key={sz} onClick={()=>{setPaperSize(sz);DB.setSetting("paper_size",sz)}} style={{padding:"6px 16px",borderRadius:8,border:paperSize===sz?"2px solid #1e40af":"1.5px solid #e2e8f0",background:paperSize===sz?"#eff6ff":"#fff",color:paperSize===sz?"#1e40af":"#64748b",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>{sz==="58mm"?"📜 58mm":sz==="80mm"?"🧾 80mm":"📄 A4"}</button>)}
</div>
...
<img src={STORE_LOGO} alt="3045" style={{height:paperSize==="A4"?60:paperSize==="80mm"?40:30,marginBottom:4}}/>
<div style={{fontSize:paperSize==="A4"?22:paperSize==="80mm"?16:13,fontWeight:900}}>{storeSettings.storeName||"3045 Supermarket"}</div>
...
if(existingProd){
  ...
  if(confirm(rtl?"المنتج غير موجود. إنشاء منتج جديد بالباركود "+code+"؟":"Product not found. Create new product with barcode "+code+"?")){
    setNP({bc:code,n:"",a:"",p:"",c:"",cat:"food",u:"pc",e:"📦",exp:"",img:null,supplier:"",initQty:"",batches:[],isPackage:false,parentBarcode:"",packSize:""});
    setAPM(true);
    e.target.value="";
  }
}
...
try{await DB.closeShift(activeShift.id,update);setCashShifts(p=>p.map(s=>s.id===activeShift.id?{...s,...update}:s));
