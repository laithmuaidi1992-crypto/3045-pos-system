...
try{
  // 1) Save the return record (omit created_by if it might fail FK)
  const returnPayload={transaction_id:returnTxn.id,receipt_no:returnTxn.rn,return_type:isFullReturn?"full":"partial",return_reason:items.map(ri=>ri.reason).filter(Boolean).join(", ")||"Customer return",total_refund:totalRefund,refund_method:"cash",status:"completed"};
  ...
  await DB.addSalesReturnItems(items.map(ri=>({return_id:ret.id,product_id:isMisc(ri)?null:(typeof ri.id==="string"&&ri.id.includes("_w")?ri.id.split("_w")[0]:ri.id),product_name:ri.n,quantity:ri.returnQty,unit_price:ri.p,line_total:+(ri.returnQty*ri.p).toFixed(3),reason:ri.reason||""})));
