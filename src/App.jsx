...
useEffect(()=>{
  ...
  async function load(){
    try{
      ...
      // Step 1: Critical data only (products + users) — these must succeed
      const[p,u]=await Promise.all([DB.getProducts(),DB.getUsers()]);
      console.log("[load] Critical OK:",{products:p.length,users:u.length});
      setProds(p);setUsers(u);setDbOk(true);
      clearTimeout(safetyTimeout);
      setLoading(false); // ← App becomes usable HERE, after critical data loads
...
useEffect(()=>{if(!loggedIn)return;const h=e=>{const tg=e.target.tagName;if(tg==="INPUT"||tg==="TEXTAREA"||tg==="SELECT")return;if(pmMod||rcMod)return;if(e.key==="Enter"){const c=bcB.current.trim();bcB.current="";if(c.length>=4){const p=prods.find(x=>x.bc===c);if(p){addToCart(p);sT("✓ "+pN(p)+" "+t.added,"ok");}else sT("✗ "+t.notFound,"err");}return;}if(e.key.length===1&&!e.ctrlKey&&!e.metaKey&&!e.altKey){bcB.current+=e.key;clearTimeout(bcTm.current);bcTm.current=setTimeout(()=>{bcB.current=""},200);}};window.addEventListener("keydown",h);return()=>window.removeEventListener("keydown",h);},[loggedIn,prods,pmMod,rcMod,lang,addToCart]);
...
{txns.slice(0,20).map(tx=><div key={tx.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,padding:12,marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
<div style={{flex:1,cursor:"pointer"}} onClick={()=>openReceiptWithItems(tx)}><div style={{fontFamily:"var(--m)",fontSize:12,fontWeight:700,color:"#1e40af"}}>{tx.rn}</div><div style={{fontSize:11,color:"#6b7280"}}>{tx.date} {tx.time} · {(tx.items||[]).length} {t.items}</div></div>
<div style={{fontFamily:"var(--m)",fontWeight:700,color:"#059669",fontSize:14}}>{fm(tx.tot)}</div>
<button onClick={async(e)=>{e.stopPropagation();let txWithItems=tx;if(!tx.items||tx.items.length===0){try{const{data:items}=await sb.from("transaction_items").select("*").eq("transaction_id",tx.id).limit(5000);if(items&&items.length>0){txWithItems={...tx,items:items.map(i=>({id:i.product_id||"misc_"+i.id,n:i.product_name||"—",a:i.product_name_ar||i.product_name||"—",bc:i.barcode||"MISC",p:+i.unit_price,qty:i.quantity,_isMisc:!i.product_id}))};setTxns(prev=>prev.map(x=>x.id===tx.id?txWithItems:x))}else{sT("✗ "+(rtl?"لا توجد بنود لهذه الفاتورة":"No items for this transaction"),"err");return}}catch(er){sT("✗ "+er.message,"err");return}}setReturnTxn(txWithItems);setReturnItems(txWithItems.items.map(i=>({...i,returnQty:0,reason:""})));setSalesReturnMod(true)}} style={{padding:"8px 14px",background:"#dc2626",border:"none",borderRadius:8,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",whiteSpace:"nowrap"}}>↩️ {rtl?"إرجاع":"Return"}</button>
</div>)}
</div>}
...
try{
  // 1) Save the return record (transaction_id column expects bigint but our IDs are text — link via receipt_no instead)
  ...
  const ret=await DB.addSalesReturn(returnPayload);
  if(!ret){throw new Error(rtl?"فشل حفظ المرتجع":"Failed to save return")}
  // 2) Save return items — set product_id to null for misc items (no FK violation)
  const isMisc=(ri)=>ri._isMisc||(typeof ri.id==="string"&&ri.id.startsWith("misc_"));
  // Detect if product_id column is bigint or text by checking sample products
  await DB.addSalesReturnItems(items.map(ri=>{
    const realId=isMisc(ri)?null:(typeof ri.id==="string"&&ri.id.includes("_w")?ri.id.split("_w")[0]:ri.id);
    // If realId is non-numeric string, set to null to avoid bigint error
    const safeId=realId&&typeof realId==="string"&&!/^\d+$/.test(realId)?null:realId;
    return{return_id:ret.id,product_id:safeId,product_name:ri.n,quantity:ri.returnQty,unit_price:ri.p,line_total:+(ri.returnQty*ri.p).toFixed(3),reason:ri.reason||""};
  }));
  // 3) Restore stock — skip misc items (no real product)
  for(const ri of items){
    ...
    }catch(er){console.error("Restore stock for "+ri.id+":",er)}
  }
  // 4) Deduct refund from Cash Register bank account
  try{
    const cashAcct=bankAccts.find(a=>a.name&&(a.name.toLowerCase().includes("cash register")||a.name.toLowerCase().includes("صندوق")));
    if(cashAcct){
      await DB.addMoneyMovement({account_id:cashAcct.id,type:"withdrawal",amount:totalRefund,reference_no:"REFUND-"+ret.id,description:(rtl?"مرتجع مبيعات للفاتورة ":"Sales return for receipt ")+returnTxn.rn,date:new Date().toISOString().slice(0,10),created_by:cu?.id});
  ...
  }catch(er){console.error("Refund deduction:",er)}
  // 5) Refresh state
  setSalesReturns(p=>[ret,...p]);
  ...
  sT("✗ "+(rtl?"فشل الإرجاع: ":"Return failed: ")+(e.message||"Unknown error"),"err");
