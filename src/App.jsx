...
<button onClick={async(e)=>{e.stopPropagation();let txWithItems=tx;if(!tx.items||tx.items.length===0){try{const{data:items}=await sb.from("transaction_items").select("*").eq("transaction_id",tx.id).limit(5000);if(items&&items.length>0){txWithItems={...tx,items:items.map(i=>({id:i.product_id,n:i.product_name,a:i.product_name_ar||i.product_name,bc:i.barcode,p:+i.unit_price,qty:i.quantity}))};setTxns(prev=>prev.map(x=>x.id===tx.id?txWithItems:x))}else{sT("✗ "+(rtl?"لا توجد بنود لهذه الفاتورة":"No items for this transaction"),"err");return}}catch(er){sT("✗ "+er.message,"err");return}}setReturnTxn(txWithItems);setReturnItems(txWithItems.items.map(i=>({...i,returnQty:0,reason:""})));setSalesReturnMod(true)}} style={{padding:"8px 14px",background:"#dc2626",border:"none",borderRadius:8,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",whiteSpace:"nowrap"}}>↩️ {rtl?"إرجاع":"Return"}</button>
</div>)}
</div>}
...
try{
  ...
  const ret=await DB.addSalesReturn({transaction_id:returnTxn.id,receipt_no:returnTxn.rn,return_type:isFullReturn?"full":"partial",return_reason:items.map(ri=>ri.reason).filter(Boolean).join(", ")||"Customer return",total_refund:totalRefund,refund_method:"cash",status:"completed",created_by:cu?.id});
  if(!ret){throw new Error(rtl?"فشل حفظ المرتجع":"Failed to save return")}
  // 2) Save return items
  await DB.addSalesReturnItems(items.map(ri=>({return_id:ret.id,product_id:ri.id,product_name:ri.n,quantity:ri.returnQty,unit_price:ri.p,line_total:+(ri.returnQty*ri.p).toFixed(3),reason:ri.reason||""})));
  // 3) Restore stock to each product (DIRECT DB update — fetch current first)
  for(const ri of items){
    try{
      const{data:cur}=await sb.from("products").select("stock").eq("id",ri.id).single();
      if(cur){
        const newStock=cur.stock+ri.returnQty;
        await sb.from("products").update({stock:newStock,updated_at:new Date().toISOString()}).eq("id",ri.id);
        setProds(prev=>prev.map(x=>x.id===ri.id?{...x,s:newStock}:x));
      }
