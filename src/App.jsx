...
<div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:12,marginBottom:10}}>
  <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:10}}>
    ...
    <select value={salesGroupBy} onChange={e=>setSalesGroupBy(e.target.value)} style={{padding:"10px 14px",background:salesGroupBy!=="none"?"#f5f3ff":"#f9fafb",border:"1.5px solid "+(salesGroupBy!=="none"?"#7c3aed":"#e5e7eb"),borderRadius:10,fontSize:11,outline:"none",minWidth:130,fontWeight:salesGroupBy!=="none"?700:400}}>
      <option value="none">📊 {rtl?"بدون تجميع":"No Grouping"}</option>
      <option value="cashier">👤 {rtl?"حسب الكاشير":"By Cashier"}</option>
      <option value="category">🏷️ {rtl?"حسب الفئة":"By Category"}</option>
      <option value="method">💳 {rtl?"حسب طريقة الدفع":"By Payment Method"}</option>
      <option value="date">📅 {rtl?"حسب التاريخ":"By Date"}</option>
      <option value="hour">🕐 {rtl?"حسب الساعة":"By Hour"}</option>
      <option value="customer">👥 {rtl?"حسب العميل":"By Customer"}</option>
      <option value="product">📦 {rtl?"حسب المنتج":"By Product"}</option>
    </select>
  </div>
...
{/* ── GROUPED VIEW ── */}
{groupedData && (
  <div style={{background:"#fff",border:"2px solid #7c3aed",borderRadius:12,marginBottom:10,overflow:"hidden"}}>
    <div style={{padding:"10px 14px",background:"linear-gradient(90deg,#f5f3ff,#fff)",borderBottom:"1px solid #e5e7eb",fontWeight:800,fontSize:13,color:"#5b21b6"}}>
      📊 {rtl?"تجميع حسب":"Grouped by"}: <span style={{textTransform:"capitalize"}}>{salesGroupBy}</span> — {groupedData.length} {rtl?"مجموعة":"groups"}
    </div>
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
      <thead style={{background:"#f9fafb"}}>
        <tr>
          <th style={{padding:"10px 14px",textAlign:"left",fontSize:11,color:"#6b7280",fontWeight:700}}>📌 {rtl?"المجموعة":"Group"}</th>
          <th style={{padding:"10px 14px",textAlign:"center",fontSize:11,color:"#6b7280",fontWeight:700}}>🧾 {rtl?"العدد":"Count"}</th>
          <th style={{padding:"10px 14px",textAlign:"right",fontSize:11,color:"#059669",fontWeight:700}}>💰 {rtl?"الإجمالي":"Total"}</th>
          <th style={{padding:"10px 14px",textAlign:"right",fontSize:11,color:"#dc2626",fontWeight:700}}>↩️ {rtl?"المسترد":"Refunded"}</th>
          <th style={{padding:"10px 14px",textAlign:"right",fontSize:11,color:"#1e40af",fontWeight:700}}>✓ {rtl?"الصافي":"Net"}</th>
          <th style={{padding:"10px 14px",textAlign:"right",fontSize:11,color:"#6b7280",fontWeight:700}}>📊 %</th>
        </tr>
      </thead>
      <tbody>
        {groupedData.map(([key,data])=>{
          const pct = filteredTotal>0 ? (data.total/filteredTotal*100) : 0;
          return <tr key={key} style={{borderTop:"1px solid #f3f4f6"}}>
            <td style={{padding:"10px 14px",fontWeight:700}}>{key}</td>
            <td style={{padding:"10px 14px",textAlign:"center",fontFamily:"monospace",fontWeight:700}}>{data.count}</td>
            <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",fontWeight:700,color:"#059669"}}>{fm(data.total)}</td>
            <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",color:data.refund>0?"#dc2626":"#d1d5db"}}>{data.refund>0?"−"+fm(data.refund):"—"}</td>
            <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",fontWeight:800,color:"#1e40af"}}>{fm(data.total-data.refund)}</td>
            <td style={{padding:"10px 14px",textAlign:"right"}}>
              <div style={{display:"inline-flex",alignItems:"center",gap:6}}>
                <span style={{fontFamily:"monospace",fontSize:11,fontWeight:700,color:"#7c3aed"}}>{pct.toFixed(1)}%</span>
      ...
      <tfoot style={{background:"#f9fafb",fontWeight:800}}>
        <tr style={{borderTop:"2px solid #7c3aed"}}>
          <td style={{padding:"10px 14px",fontWeight:800,color:"#5b21b6"}}>📊 {rtl?"الإجمالي":"TOTAL"}</td>
          <td style={{padding:"10px 14px",textAlign:"center",fontFamily:"monospace",color:"#5b21b6"}}>{sorted.length}</td>
          <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",color:"#059669"}}>{fm(filteredTotal)}</td>
          <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",color:"#dc2626"}}>−{fm(filteredRefund)}</td>
          <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",color:"#1e40af"}}>{fm(filteredNet)}</td>
          <td style={{padding:"10px 14px",textAlign:"right",fontFamily:"monospace",color:"#5b21b6"}}>100%</td>
        </tr>
      </tfoot>
    </table>
  </div>
