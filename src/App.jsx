...
useEffect(()=>{
  // Safety net: force loading=false after 15s no matter what
  const safetyTimeout=setTimeout(()=>{console.warn("⚠ Load timeout — forcing app to continue");setLoading(false);setDbOk(false)},15000);
  async function load(){
    try{
      ...
      console.log("[load] Core data OK:",{products:p.length,users:u.length});
    ...
    setLoading(false);
    console.log("[load] Done, app ready");
  }
  load();
},[]);
