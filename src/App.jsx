...
const DB = {
  ...
  async getProducts() {
    ...
    return all.map(r=>({id:r.id,bc:r.barcode,n:r.name,a:r.name_ar,p:+r.price,c:+r.cost,cat:r.category,u:r.unit,s:r.stock,e:r.emoji,exp:r.expiry_date||null,img:r.image||null}));
  },
...
const[paperSize,setPaperSize]=useState(()=>localStorage.getItem("3045_paper")||"80mm");
...
const[toast,setToast]=useState(null);const[voiceOn,setVoiceOn]=useState(()=>localStorage.getItem("3045_voice")==="true");
const speak=(text)=>{if(!voiceOn||!window.speechSynthesis)return;const u=new SpeechSynthesisUtterance(text);u.lang="ar-SA";u.rate=1;u.pitch=1;window.speechSynthesis.speak(u)};const[pwMod,setPWM]=useState(null);const[nPW,setNPW]=useState("");
const[auMod,setAUM]=useState(false);const[nU,setNU]=useState({un:"",fn:"",fa:"",role:"cashier",pw:""});
const[apMod,setAPM]=useState(false);const[nP,setNP]=useState({bc:"",n:"",a:"",p:"",c:"",cat:"food",u:"pc",e:"📦",exp:"",img:null,supplier:"",initQty:"",batches:[],isPackage:false,parentBarcode:"",packSize:""});
...
const[invs,setInvs]=useState([]);const[suppliers,setSuppliers]=useState(()=>{try{return JSON.parse(localStorage.getItem('3045_suppliers')||'[]')}catch{return[]}});const[invMod,setInvMod]=useState(false);const[invView,setInvView]=useState(null);
const[invSup,setInvSup]=useState("");const[invNo,setInvNo]=useState("");
const[invItems,setInvItems]=useState([{prodId:"",qty:"",cost:"",barcode:"",name:"",expiry:""}]);
...
const[storeSettings,setStoreSettings]=useState(()=>{try{return JSON.parse(localStorage.getItem("3045_settings"))||{storeName:"3045 Supermarket",taxRate:0,currency:"JD",dailyTarget:500,weeklyTarget:3000,monthlyTarget:12000}}catch{return{storeName:"3045 Supermarket",taxRate:0,currency:"JD",dailyTarget:500,weeklyTarget:3000,monthlyTarget:12000}}});
...
const[stocktakeItems,setStocktakeItems]=useState([]);const[stocktakeSearch,setStocktakeSearch]=useState("");
...
// Weight-based products — prompt for weight
if(p.u==="kg"||p.u==="g"){
  const w=prompt(rtl?"أدخل الوزن (كغ):":"Enter weight (kg):","1");
  if(!w)return;const wt=parseFloat(w);if(isNaN(wt)||wt<=0)return;
  beep();speak((p.a||p.n)+" "+wt+" "+(rtl?"كيلو":"kilo"));
  setCart(prev=>{const ex=prev.find(i=>i.id===p.id+"_w"+Date.now());return[...prev,{...p,id:p.id+"_w"+Date.now(),qty:1,p:+(p.p*wt).toFixed(3),_weight:wt,_origPrice:p.p}]});
  return;
}
beep();speak(p.a||p.n);
// Check if product is near expiry — auto-suggest discount
const pBatches2=batches.filter(b=>b.product_id===p.id&&b.status==="active"&&b.quantity_remaining>0);
const nearestExp=pBatches2.length>0?pBatches2.sort((a,b)=>new Date(a.expiry_date)-new Date(b.expiry_date))[0]:null;
if(nearestExp&&nearestExp.expiry_date){const dLeft=Math.ceil((new Date(nearestExp.expiry_date)-new Date())/86400000);if(dLeft<=7&&dLeft>0){sT("⚠️ "+(rtl?"قارب على الانتهاء! ":"Near expiry! ")+dLeft+(rtl?" يوم — خصم مقترح 30%":"d left — suggest 30% off"),"err")}}
setCart(prev=>{const ex=prev.find(i=>i.id===p.id);return ex?prev.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i):[...prev,{...p,qty:1}]})},[beep]);
...
const cP=async()=>{
speak((rtl?"الإجمالي ":"Total ")+fN(selCust&&redeemPts>0?totAfterRedeem:tot)+(rtl?" دينار":" JD"));
  ...
  try{
    ...
    // Package deduction — if item is a package, also deduct from parent product
    const pkgs=JSON.parse(localStorage.getItem("3045_packages")||"{}");
    for(const item of cart){
      const pkg=pkgs[item.bc];
      if(pkg){
        const deductQty=item.qty*pkg.packSize;
        try{
          const{data:parent}=await sb.from("products").select("stock").eq("id",pkg.parentId).single();
          if(parent){await sb.from("products").update({stock:Math.max(0,parent.stock-deductQty)}).eq("id",pkg.parentId)}
          await DB.deductBatchFIFO(pkg.parentId,deductQty);
        }catch{}
      }
    }
...
const saveInv=async()=>{
  ...
  // Group items by prodId and sum quantities
const grouped={};vi.forEach(x=>{if(!grouped[x.prodId])grouped[x.prodId]={qty:0,cost:0,items:[]};grouped[x.prodId].qty+=parseInt(x.qty)||0;grouped[x.prodId].cost=parseFloat(x.cost)||0;grouped[x.prodId].items.push(x)});
setProds(p=>p.map(pr=>{const g=grouped[pr.id];return g?{...pr,s:pr.s+g.qty,c:g.cost}:pr}));
// Create expiry batches for items with expiry dates
for(const pid of Object.keys(grouped)){
  for(const item of grouped[pid].items){
    if(item.expiry){
      try{await DB.addBatch({product_id:pid,batch_number:"B-"+Date.now().toString(36)+Math.random().toString(36).slice(2,5),supplier_name:invSup,received_date:new Date().toISOString().slice(0,10),expiry_date:item.expiry,quantity_received:parseInt(item.qty)||0,quantity_remaining:parseInt(item.qty)||0,cost_per_unit:parseFloat(item.cost)||0,notes:"Invoice: "+invNo})}catch(e){console.error(e)}
    }
  }
}
...
const ctD=useMemo(()=>{
  const catMap={"chocolate":"Chocolate","biscuits":"Biscuits","cake":"Cake","candy":"Candy","snacks":"Snacks","nuts":"Nuts","soda":"Soda","juice":"Juice","energy":"Energy","water":"Water","coffee":"Coffee","dairy":"Dairy","meat":"Meat","food":"Food","frozen":"Frozen","breakfast":"Breakfast","baby":"Baby","personal":"Personal","household":"Household","cigarettes":"Cigarettes","electronics":"Electronics"};
  const catMapAr={"chocolate":"شوكولاتة","biscuits":"بسكويت","cake":"كيك","candy":"حلويات","snacks":"شبس","nuts":"مكسرات","soda":"غازية","juice":"عصائر","energy":"طاقة","water":"مياه","coffee":"قهوة","dairy":"ألبان","meat":"لحوم","food":"أغذية","frozen":"مجمدات","breakfast":"فطور","baby":"أطفال","personal":"عناية","household":"منزلية","cigarettes":"سجائر","electronics":"إلكترونيات"};
  const cats={};
...
.mn{flex-direction:column}.pp{height:auto;min-height:0}.cp{width:100%;max-height:40vh;border-left:none;border-top:1px solid var(--g200)}
.pg{grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:6px;padding:8px}
.pc{padding:10px 8px}.pe{font-size:24px}.pn{font-size:11px}
.cats{padding:6px 8px;gap:4px}.ch{padding:6px 10px;font-size:11px}
.sb{padding:8px}.si{font-size:12px;padding:10px 14px 10px 34px}
.pbs{padding:8px 10px;gap:4px}.pb{padding:14px 8px;font-size:12px}
.nav{padding:0}.nt{padding:6px 10px;font-size:11px}
.hdr{padding:6px 10px}.logo-m{width:28px;height:28px;font-size:10px}.logo-t{font-size:14px}
.at{font-size:12px}
.ch2 h3{font-size:13px}.csm{padding:10px}.sr{font-size:12px}.sr.T{font-size:14px}
.ad{flex-direction:column}.ads{flex-direction:row;overflow-x:auto;flex-wrap:nowrap;padding:8px}.asb{white-space:nowrap;font-size:11px;padding:6px 10px}
.ac{padding:10px}
}
@media(max-width:480px){
.pg{grid-template-columns:repeat(3,1fr)}.cp{max-height:35vh}
.hdr{padding:4px 8px}.hb{font-size:9px;padding:4px 8px}
.pbs{flex-wrap:wrap}.pb{min-width:45%}
}
...
<div style={{fontSize:15,fontWeight:700,color:"#374151",marginBottom:2,direction:rtl?"rtl":"ltr"}}>{t.attendance}</div>
...
<button onClick={()=>{const csv=["Product,Barcode,Stock,Suggested Order"];lowItems.forEach(p=>csv.push('"'+pN(p)+'","'+p.bc+'",'+p.s+","+(50-p.s)));const b=new Blob([csv.join("\n")],{type:"text/csv"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="reorder-"+new Date().toISOString().slice(0,10)+".csv";a.click()}} style={{padding:"4px 12px",background:"#dc2626",border:"none",borderRadius:8,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>📥 {rtl?"تصدير طلب":"Export Order"}</button>
</div>
...
<div style={{fontSize:14,fontWeight:700,color:"#374151"}}>📊 {t.today}</div>
...
{/* Purchase Patterns — Top Sellers */}
{txns.length>0&&<div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,padding:16}}>
<div style={{fontSize:14,fontWeight:700,marginBottom:10}}>🔥 {rtl?"الأكثر مبيعاً هذا الأسبوع":"Top Sellers This Week"}</div>
{(()=>{const week=txns.filter(tx=>{try{const d=new Date();d.setDate(d.getDate()-7);return new Date(tx.ts)>=d}catch{return false}});
const pm={};week.forEach(tx=>tx.items.forEach(i=>{if(!pm[i.id])pm[i.id]={name:pN(i),qty:0,rev:0};pm[i.id].qty+=i.qty;pm[i.id].rev+=i.p*i.qty}));
const top=Object.values(pm).sort((a,b)=>b.qty-a.qty).slice(0,5);
return top.length>0?top.map((p,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:i<top.length-1?"1px solid #f3f4f6":"none"}}>
...
</div>):<div style={{textAlign:"center",color:"#9ca3af",fontSize:12,padding:10}}>{rtl?"لا بيانات":"No data yet"}</div>})()}
</div>}
...
<div className="tb" style={{minHeight:0}}><div className="tbh"><span>🕐 {t.recentActivity}</span></div><div style={{maxHeight:250,overflowY:"auto"}}><table><thead><tr><th>{t.receipt}</th><th>{t.time}</th><th>👤</th><th>{t.method}</th><th>{t.total}</th></tr></thead><tbody>{txns.slice(0,20).map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}><td className="mn" style={{fontSize:11}}>{tx.rn}</td><td style={{fontSize:11}}>{tx.date} {tx.time}</td><td style={{fontSize:11,color:tx.cashierName?"#1e40af":"#d1d5db"}}>{tx.cashierName||"—"}</td><td><span style={{padding:"2px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:tx.method==="cash"?"#ecfdf5":tx.method==="card"?"#eff6ff":"#fff7ed",color:tx.method==="cash"?"#059669":tx.method==="card"?"#2563eb":"#ea580c"}}>{tx.method==="mobile"?t.mada:tx.method==="card"?t.card:t.cash}</span></td><td className="mn" style={{color:"#059669"}}>{fm(tx.tot)}</td></tr>)}</tbody></table></div></div>
</div>})()}
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:c.status==="active"?"#ecfdf5":"#fef2f2",color:c.status==="active"?"#059669":"#dc2626"}}>{c.status}</span>
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:s.status==="paid"?"#ecfdf5":s.status==="pending"?"#fffbeb":"#fef2f2",color:s.status==="paid"?"#059669":s.status==="pending"?"#d97706":"#dc2626"}}>{t[s.status]}</span>
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:l.status==="approved"?"#ecfdf5":l.status==="pending"?"#fffbeb":"#fef2f2",color:l.status==="approved"?"#059669":l.status==="pending"?"#d97706":"#dc2626"}}>{t[l.status]}</span>
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:a.status==="present"?"#ecfdf5":a.status==="late"?"#fffbeb":"#fef2f2",color:a.status==="present"?"#059669":a.status==="late"?"#d97706":"#dc2626"}}>{t[a.status]||a.status}</span>
...
<td style={{fontFamily:"var(--m)",fontSize:11}}>{s.month}/{s.year}</td>
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:s.status==="paid"?"#ecfdf5":"#fffbeb",color:s.status==="paid"?"#059669":"#d97706"}}>{t[s.status]}</span></td>
</tr>})}</tbody></table></div>
</>})()}
</div></div>}
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:isIn?"#ecfdf5":"#fef2f2",color:isIn?"#059669":"#dc2626"}}>{isIn?"↑":"↓"} {m.type}</span></td>
...
<div style={{fontSize:18,fontWeight:800,color:"#1f2937"}}>{storeSettings.storeName||"3045 Supermarket"}</div>
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:s.status==="open"?"#ecfdf5":s.status==="closed"?"#eff6ff":"#f3f4f6",color:s.status==="open"?"#059669":s.status==="closed"?"#2563eb":"#6b7280"}}>{s.status}</span></td>
...
// Today's purchases
const todayInvs=invs.filter(inv=>{try{return inv.date===todayStr||new Date(inv.date).toDateString()===new Date().toDateString()}catch{return false}});
const todayPurchases=todayInvs.reduce((s,inv)=>s+inv.totalCost,0);
// Cash movements today
const todayCashIn=cashOps.filter(o=>o.type==="in").reduce((s,o)=>s+o.amount,0);
const todayCashOut=cashOps.filter(o=>o.type==="out").reduce((s,o)=>s+o.amount,0);
// Remittance = Cash sales + Cash In - Cash Out - Opening balance (keep opening in register)
const openingBal=activeShift?+activeShift.opening_balance:0;
const remittance=Math.max(0,cashS+todayCashIn-todayCashOut-openingBal);

return<><h2 style={{fontSize:20}}>📄 {rtl?"تقرير نهاية اليوم":"End of Day Report"}</h2>
...
try{const r=await DB.addEODReport(report);if(r)setEODReports(p=>{const existing=p.findIndex(x=>x.report_date===todayStr);if(existing>=0){const n=[...p];n[existing]=r;return n}return[r,...p]});sT("✓ "+(rtl?"تم إنشاء التقرير":"Report saved"),"ok")}catch(e){console.error(e)}}}>💾 {rtl?"حفظ التقرير":"Save Report"}</button>
<button className="ab ab-x" style={{padding:"10px 20px",fontSize:13}} onClick={()=>{const el=document.getElementById("eod-print");if(!el)return;const win=window.open("","_blank");win.document.write("<!DOCTYPE html><html><head><meta charset='utf-8'><title>EOD Report</title><style>@page{size:A4;margin:15mm}*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:12pt;direction:"+(rtl?"rtl":"ltr")+"}table{width:100%;border-collapse:collapse}th,td{padding:6px 8px;border-bottom:1px solid #ddd;text-align:"+(rtl?"right":"left")+"}th{background:#f5f5f5;font-weight:bold}.section{margin:16px 0;padding:12px;border:1px solid #ddd;border-radius:8px}.total{font-size:16pt;font-weight:bold;border-top:2px solid #000}.header{text-align:center;margin-bottom:20px}.r{text-align:"+(rtl?"left":"right")+"}</style></head><body>");win.document.write(el.innerHTML);win.document.write("</body></html>");win.document.close();setTimeout(()=>{win.print();setTimeout(()=>win.close(),500)},300)}}>🖨 {t.print}</button>
</div>

{/* ═══ EOD REPORT CARD ═══ */}
<div id="eod-print" style={{background:"#fff",border:"1.5px solid #e2e8f0",borderRadius:20,padding:28,maxWidth:600,margin:"0 auto"}}>

{/* Header */}
<div style={{textAlign:"center",marginBottom:20,paddingBottom:16,borderBottom:"3px double #1e293b"}}>
<img src={STORE_LOGO} alt="3045" style={{height:50,objectFit:"contain",marginBottom:6}}/><div style={{fontSize:22,fontWeight:800}}>{storeSettings.storeName||"3045 Supermarket"}</div>
<div style={{fontSize:11,color:"#6b7280"}}>{rtl?"إربد، شارع المدينة المنورة - مقابل SOS":"Irbid, Almadina Almonawarah St. (Opp. SOS)"} · 📞 0791191244</div>
<div style={{fontSize:18,fontWeight:800,color:"#1e40af",marginTop:10}}>{rtl?"تقرير نهاية اليوم":"END OF DAY REPORT"}</div>
<div style={{fontSize:14,color:"#374151",marginTop:4}}>{new Date().toLocaleDateString(rtl?"ar":"en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}</div>
</div>

{/* 1. Sales Summary */}
<div style={{background:"#f0fdf4",borderRadius:14,padding:16,marginBottom:14,border:"1px solid #bbf7d0"}}>
<div style={{fontSize:15,fontWeight:800,color:"#065f46",marginBottom:10}}>📊 {rtl?"ملخص المبيعات":"Sales Summary"}</div>
<table style={{width:"100%",fontSize:14}}><tbody>
<tr><td style={{padding:"6px 0"}}>💵 {t.cash}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600}}>{fN(cashS)}</td></tr>
<tr><td style={{padding:"6px 0"}}>💳 {t.card}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600}}>{fN(cardS)}</td></tr>
<tr><td style={{padding:"6px 0"}}>🏦 {t.mada}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600}}>{fN(madaS)}</td></tr>
<tr style={{borderTop:"2px solid #059669"}}><td style={{padding:"8px 0",fontWeight:800,fontSize:16}}>{t.totalSales}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:800,fontSize:18,color:"#059669"}}>{fN(totalS)}</td></tr>
<tr><td style={{padding:"4px 0",color:"#6b7280",fontSize:12}} colSpan={2}>{t.txns}: {todayTxs3.length} · {t.items}: {totalItems}</td></tr>
</tbody></table>
</div>

{/* 2. Today's Purchases */}
<div style={{background:"#eff6ff",borderRadius:14,padding:16,marginBottom:14,border:"1px solid #bfdbfe"}}>
<div style={{fontSize:15,fontWeight:800,color:"#1e40af",marginBottom:10}}>🧾 {rtl?"مشتريات اليوم":"Today's Purchases"}</div>
{todayInvs.length===0?<div style={{textAlign:"center",color:"#6b7280",fontSize:13,padding:8}}>{rtl?"لا مشتريات اليوم":"No purchases today"}</div>:
<table style={{width:"100%",fontSize:13}}><tbody>
{todayInvs.map((inv,i)=><tr key={i}><td style={{padding:"4px 0"}}>📦 {inv.supplier} ({inv.invoiceNo})</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600,color:"#dc2626"}}>{fN(inv.totalCost)}</td></tr>)}
<tr style={{borderTop:"2px solid #1e40af"}}><td style={{padding:"8px 0",fontWeight:800}}>{rtl?"إجمالي المشتريات":"Total Purchases"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:800,color:"#dc2626"}}>{fN(todayPurchases)}</td></tr>
</tbody></table>}
</div>

{/* 3. Cash Movement */}
<div style={{background:"#fffbeb",borderRadius:14,padding:16,marginBottom:14,border:"1px solid #fde68a"}}>
<div style={{fontSize:15,fontWeight:800,color:"#92400e",marginBottom:10}}>💰 {rtl?"حركة النقد":"Cash Movement"}</div>
<table style={{width:"100%",fontSize:14}}><tbody>
<tr><td style={{padding:"5px 0"}}>{rtl?"الرصيد الافتتاحي":"Opening Balance"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600}}>{fN(openingBal)}</td></tr>
<tr><td style={{padding:"5px 0",color:"#059669"}}>↑ {rtl?"مبيعات نقدية":"Cash Sales"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600,color:"#059669"}}>+{fN(cashS)}</td></tr>
{todayCashIn>0&&<tr><td style={{padding:"5px 0",color:"#059669"}}>↑ {rtl?"إدخال نقد":"Cash In"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600,color:"#059669"}}>+{fN(todayCashIn)}</td></tr>}
{todayCashOut>0&&<tr><td style={{padding:"5px 0",color:"#dc2626"}}>↓ {rtl?"إخراج نقد":"Cash Out"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600,color:"#dc2626"}}>-{fN(todayCashOut)}</td></tr>}
<tr style={{borderTop:"2px solid #92400e"}}><td style={{padding:"8px 0",fontWeight:800}}>{rtl?"المتوقع في الصندوق":"Expected in Drawer"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:800}}>{fN(openingBal+cashS+todayCashIn-todayCashOut)}</td></tr>
</tbody></table>
</div>

{/* 4. Expenses */}
{todayExp>0&&<div style={{background:"#fef2f2",borderRadius:14,padding:16,marginBottom:14,border:"1px solid #fecaca"}}>
<div style={{fontSize:15,fontWeight:800,color:"#991b1b",marginBottom:10}}>💸 {rtl?"مصروفات اليوم":"Today's Expenses"}</div>
<table style={{width:"100%",fontSize:13}}><tbody>
{expensesList.filter(e=>e.expense_date===todayStr).map((e,i)=>{const cat=expCats.find(c=>c.id===e.category_id);return<tr key={i}><td style={{padding:"4px 0"}}>{cat?(cat.icon+" "+(rtl?cat.name_ar:cat.name)):"—"}: {e.description||"—"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600,color:"#dc2626"}}>{fN(+e.amount)}</td></tr>})}
<tr style={{borderTop:"2px solid #dc2626"}}><td style={{padding:"6px 0",fontWeight:800}}>{t.totalExpenses}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:800,color:"#dc2626"}}>{fN(todayExp)}</td></tr>
</tbody></table>
</div>}

{/* 5. Profitability */}
<div style={{background:"#f8fafc",borderRadius:14,padding:16,marginBottom:14,border:"1px solid #e2e8f0"}}>
<div style={{fontSize:15,fontWeight:800,color:"#374151",marginBottom:10}}>📈 {rtl?"الربحية":"Profitability"}</div>
<table style={{width:"100%",fontSize:14}}><tbody>
<tr><td style={{padding:"5px 0"}}>{t.grossRevenue}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:600}}>{fN(totalS)}</td></tr>
<tr><td style={{padding:"5px 0",color:"#dc2626"}}>{t.costOfGoods}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",color:"#dc2626"}}>-{fN(todayCogs)}</td></tr>
<tr style={{borderTop:"1px solid #e2e8f0"}}><td style={{padding:"5px 0",fontWeight:700}}>{t.grossProfit}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:700,color:gProf>=0?"#059669":"#dc2626"}}>{fN(gProf)} ({gMargin}%)</td></tr>
{todayExp>0&&<tr><td style={{padding:"5px 0",color:"#dc2626"}}>💸 {t.expenses}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",color:"#dc2626"}}>-{fN(todayExp)}</td></tr>}
{todayRet>0&&<tr><td style={{padding:"5px 0",color:"#dc2626"}}>↩️ {rtl?"مرتجعات":"Returns"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",color:"#dc2626"}}>-{fN(todayRet)}</td></tr>}
<tr style={{borderTop:"2px solid #1e293b"}}><td style={{padding:"8px 0",fontWeight:800,fontSize:16}}>💎 {t.netProfit}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:800,fontSize:18,color:nProf>=0?"#059669":"#dc2626"}}>{fN(nProf)}</td></tr>
</tbody></table>
</div>

{/* 6. REMITTANCE — Amount to deliver to management */}
<div style={{background:"linear-gradient(135deg,#1e3a5f,#2563eb)",borderRadius:14,padding:20,marginBottom:14,color:"#fff"}}>
<div style={{fontSize:15,fontWeight:800,marginBottom:10}}>🏦 {rtl?"المبلغ الواجب توريده للإدارة":"Amount to Remit to Management"}</div>
<table style={{width:"100%",fontSize:14,color:"#fff"}}><tbody>
<tr><td style={{padding:"5px 0",opacity:.8}}>💵 {rtl?"مبيعات نقدية":"Cash Sales"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)"}}>{fN(cashS)}</td></tr>
{todayCashIn>0&&<tr><td style={{padding:"5px 0",opacity:.8}}>↑ {rtl?"إدخال نقد":"Cash In"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)"}}>+{fN(todayCashIn)}</td></tr>}
{todayCashOut>0&&<tr><td style={{padding:"5px 0",opacity:.8}}>↓ {rtl?"إخراج نقد":"Cash Out"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)"}}>-{fN(todayCashOut)}</td></tr>}
<tr><td style={{padding:"5px 0",opacity:.8}}>🔒 {rtl?"الافتتاحي (يبقى)":"Opening (stays)"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)"}}>-{fN(openingBal)}</td></tr>
<tr style={{borderTop:"2px solid rgba(255,255,255,.3)"}}><td style={{padding:"10px 0",fontWeight:900,fontSize:20}}>{rtl?"المبلغ المطلوب توريده":"REMITTANCE DUE"}</td><td style={{textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:900,fontSize:24}}>{fN(remittance)}</td></tr>
</tbody></table>
</div>

{/* Footer */}
<div style={{textAlign:"center",marginTop:16,paddingTop:12,borderTop:"3px double #e2e8f0",fontSize:12,color:"#9ca3af"}}>
{rtl?"تم الإنشاء بواسطة":"Generated by"}: {cu?.fn} · {new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}
</div>
</div>
...
<span style={{padding:"2px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:(typeColors[doc.type]||"#6b7280")+"15",color:typeColors[doc.type]||"#6b7280"}}>{t[doc.type==="rent"?"rentContract":doc.type==="license"?"license":doc.type==="insurance"?"insurance":doc.type==="agreement"?"agreement":"otherDoc"]}</span>
</div>
</div>
...
<button onClick={()=>{setAPM(true);setNP({bc:"",n:"",a:"",p:"",c:"",cat:"food",u:"pc",e:"📦",exp:"",img:null,supplier:"",initQty:"",batches:[],isPackage:false,parentBarcode:"",packSize:""})}} style={{padding:"10px 14px",background:"#059669",border:"none",borderRadius:10,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>+</button>
...
{CATS.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:"14px 6px",background:cat===c.id?"#1e40af":"#fff",border:cat===c.id?"none":"1.5px solid #e2e8f0",borderRadius:10,fontSize:13,fontWeight:700,color:cat===c.id?"#fff":"#475569",cursor:"pointer",fontFamily:"var(--f)",textAlign:"center",lineHeight:1.3,transition:"all .15s"}}>{c.i}<br/>{rtl?t[c.k]:t[c.k]}</button>)}
</div>
...
{fpVisible.map(p=><div key={p.id} onClick={()=>addToCart(p)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#fff",border:"1px solid #f1f5f9",borderRadius:8,cursor:"pointer",transition:"all .1s"}} onMouseEnter={e=>{e.currentTarget.style.background="#eff6ff";e.currentTarget.style.borderColor="#bfdbfe"}} onMouseLeave={e=>{e.currentTarget.style.background="#fff";e.currentTarget.style.borderColor="#f1f5f9"}}>
<span style={{fontSize:16,width:24,textAlign:"center",flexShrink:0}}>{p.e}</span>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:14,fontWeight:600,color:p.s<=0?"#dc2626":"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pN(p)}</div>
<div style={{fontSize:9,color:"#94a3b8",fontFamily:"var(--m)"}}>{p.bc}</div>
</div>
<div style={{fontFamily:"var(--m)",fontSize:15,fontWeight:800,color:p.s<=0?"#dc2626":"#1e40af",flexShrink:0}}>{fN(p.p)}</div>{p.s<=0&&<span style={{fontSize:8,background:"#fef2f2",color:"#dc2626",padding:"1px 6px",borderRadius:6,fontWeight:700,border:"1px solid #fecaca"}}>{rtl?"نفذ":"OUT"}</span>}
</div>)}
...
<tbody>{cart.map((i,idx)=><tr key={i.id} style={{borderBottom:"1px solid #f1f5f9",animation:"sIn .15s ease"}}>
...
<button onClick={()=>{setPM("mobile");setCT("")}} disabled={!cart.length} style={{flex:"1 1 80px",padding:"12px 8px",background:"#ea580c",border:"none",borderRadius:10,color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",opacity:cart.length?"1":".3",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
...
{l:rtl?"فيزا/كليك":"Visa/CliQ",v:txns.filter(tx=>(tx.method==="card"||tx.method==="mobile")&&(()=>{try{return new Date(tx.ts).toDateString()===new Date().toDateString()}catch{return false}})()).length,c:"#2563eb",bg:"#eff6ff",i:"💳"},
...
<tbody>{txns.slice(0,30).map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}>
...
<td><span style={{padding:"2px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:tx.method==="cash"?"#ecfdf5":tx.method==="card"?"#eff6ff":"#fff7ed",color:tx.method==="cash"?"#059669":tx.method==="card"?"#2563eb":"#ea580c"}}>{tx.method==="mobile"?t.mada:tx.method==="card"?t.card:t.cash}</span></td>
...
<td><button className="ab ab-e" onClick={e=>{e.stopPropagation();setRM(tx)}}>🖨</button></td>
</tr>)}</tbody></table>}
</div>}
...
<td><span style={{padding:"2px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:p.s<=0?"#fef2f2":p.s<10?"#fffbeb":"#ecfdf5",color:p.s<=0?"#dc2626":p.s<10?"#d97706":"#059669"}}>{p.s<=0?(rtl?"نفذ":"Out"):p.s<10?(rtl?"منخفض":"Low"):(rtl?"متوفر":"OK")}</span></td>
</tr>)}</tbody></table>})()}
...
const totalSales=cashSales+cardSales+madaSales;
const expectedCash=+activeShift.opening_balance+cashSales;
const diff=actualCash-expectedCash;
const totalItems=todayTx.reduce((s,tx)=>s+tx.items.reduce((a,i)=>a+i.qty,0),0);
const remit=Math.max(0,cashSales-0);
const u={shift_end:new Date().toISOString(),total_cash_sales:+cashSales.toFixed(3),total_card_sales:+cardSales.toFixed(3),total_mada_sales:+madaSales.toFixed(3),expected_cash:+expectedCash.toFixed(3),actual_cash:actualCash,cash_difference:+diff.toFixed(3),difference_type:diff>0.01?"overage":diff<-0.01?"shortage":"match",total_transactions:todayTx.length,total_items_sold:totalItems,status:"closed"};
try{await DB.closeShift(activeShift.id,u);setCashShifts(p=>p.map(s=>s.id===activeShift.id?{...s,...u}:s));
// Generate printable shift report
const win=window.open("","_blank","width=400,height=600");
const d=rtl?"rtl":"ltr";
win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Shift Report</title>
<style>@page{size:80mm auto;margin:3mm}*{margin:0;padding:0;box-sizing:border-box}body{font-family:monospace;font-size:10pt;direction:${d};padding:5mm}
.c{text-align:center}.b{font-weight:bold}.row{display:flex;justify-content:space-between;padding:2px 0}.line{border-top:1px dashed #000;margin:6px 0}.big{font-size:14pt;font-weight:900}
</style></head><body>
<div class="c"><div class="b" style="font-size:14pt">${storeSettings.storeName||"3045 Supermarket"}</div>
<div style="font-size:8pt;color:#666">${rtl?"إربد، شارع المدينة المنورة - مقابل SOS":"Irbid, Almadina Almonawarah St. (Opp. SOS)"} · 0791191244</div>
<div class="b" style="font-size:12pt;margin:8px 0">${rtl?"تقرير نهاية الوردية":"SHIFT REPORT"}</div></div>
<div class="line"></div>
<div class="row"><span>${rtl?"الكاشير":"Cashier"}</span><span class="b">${activeShift.cashier_name}</span></div>
<div class="row"><span>${rtl?"بداية":"Start"}</span><span>${new Date(activeShift.shift_start).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}</span></div>
<div class="row"><span>${rtl?"نهاية":"End"}</span><span>${new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}</span></div>
<div class="row"><span>${rtl?"التاريخ":"Date"}</span><span>${new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</span></div>
<div class="line"></div>
<div class="c b" style="font-size:11pt">${rtl?"ملخص المبيعات":"Sales Summary"}</div>
<div class="row"><span>💵 ${rtl?"نقدي":"Cash"}</span><span>${cashSales.toFixed(3)}</span></div>
<div class="row"><span>💳 ${rtl?"فيزا":"Visa"}</span><span>${cardSales.toFixed(3)}</span></div>
<div class="row"><span>🏦 ${rtl?"كليك":"CliQ"}</span><span>${madaSales.toFixed(3)}</span></div>
<div class="line"></div>
<div class="row big"><span>${rtl?"الإجمالي":"TOTAL"}</span><span>${totalSales.toFixed(3)}</span></div>
<div class="row"><span>${rtl?"عدد المعاملات":"Transactions"}</span><span>${todayTx.length}</span></div>
<div class="row"><span>${rtl?"عدد الأصناف":"Items Sold"}</span><span>${totalItems}</span></div>
<div class="line"></div>
<div class="c b" style="font-size:11pt">${rtl?"حركة النقد":"Cash Reconciliation"}</div>
<div class="row"><span>${rtl?"الافتتاحي":"Opening"}</span><span>${(+activeShift.opening_balance).toFixed(3)}</span></div>
<div class="row"><span>${rtl?"مبيعات نقدية":"+ Cash Sales"}</span><span>+${cashSales.toFixed(3)}</span></div>
<div class="line"></div>
<div class="row b"><span>${rtl?"المتوقع":"Expected"}</span><span>${expectedCash.toFixed(3)}</span></div>
<div class="row b"><span>${rtl?"الفعلي":"Actual"}</span><span>${actualCash.toFixed(3)}</span></div>
<div class="line" style="border-top:2px solid #000"></div>
<div class="row big" style="color:${diff>0.01?"green":diff<-0.01?"red":"black"}"><span>${rtl?"الفرق":"Difference"}</span><span>${diff>0?"+":""}${diff.toFixed(3)} ${diff>0.01?(rtl?"فائض":"OVER"):diff<-0.01?(rtl?"عجز":"SHORT"):(rtl?"مطابق":"MATCH")}</span></div>
<div class="line"></div>
<div class="c b" style="font-size:12pt;margin:8px 0">${rtl?"المبلغ الواجب توريده":"REMITTANCE DUE"}</div>
<div class="c big" style="font-size:18pt">${remit.toFixed(3)} JD</div>
<div class="line"></div>
<div class="c" style="font-size:8pt;color:#999;margin-top:8px">${rtl?"توقيع الكاشير":"Cashier Signature"}: _______________</div>
<div class="c" style="font-size:8pt;color:#999;margin-top:4px">${rtl?"توقيع المدير":"Manager Signature"}: _______________</div>
</body></html>`);
win.document.close();setTimeout(()=>{win.print();setTimeout(()=>win.close(),500)},300);
setActiveShift(null);setShiftCashCount("");sT("✓ "+(rtl?"تم إغلاق الوردية + طباعة التقرير":"Shift closed + report printed"),"ok")}catch(e){console.error(e)}
}}>🔒 {rtl?"إغلاق + طباعة التقرير":"Close + Print Report"}</button>
</div>
...
<div style={{display:"flex",gap:8}}>
<input id="return-receipt" placeholder={rtl?"رقم الإيصال":"Receipt number"} style={{flex:1,padding:"10px 14px",border:"1.5px solid #fde68a",borderRadius:10,fontSize:14,fontFamily:"var(--m)",outline:"none"}}/>
<button onClick={()=>{const rn=document.getElementById("return-receipt")?.value;if(!rn)return;const tx=txns.find(x=>x.rn===rn);if(!tx){sT("✗ "+(rtl?"الإيصال غير موجود":"Receipt not found"),"err");return}setRM(tx);sT("✓ "+(rtl?"تم العثور":"Found")+": "+rn,"ok")}} style={{padding:"10px 20px",background:"#d97706",border:"none",borderRadius:10,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>🔍</button>
</div>
</div>
<div style={{fontSize:13,fontWeight:700,marginBottom:8}}>{rtl?"آخر المبيعات — اضغط للإرجاع":"Recent Sales — tap to return"}</div>
{txns.slice(0,20).map(tx=><div key={tx.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,padding:12,marginBottom:6,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}} onClick={()=>setRM(tx)}>
...
{/* Closure History — part of regClose tab */}
{tab==="regClose"&&<div style={{padding:16}}>
...
<table className="at"><thead><tr><th>{t.expDate}</th><th>👤</th><th>{rtl?"افتتاحي":"Opening"}</th><th>💵 {rtl?"نقدي":"Cash"}</th><th>💳 {rtl?"فيزا":"Visa"}</th><th>{rtl?"متوقع":"Expected"}</th><th>{rtl?"فعلي":"Actual"}</th><th>{rtl?"الفرق":"Diff"}</th></tr></thead>
...
{/* Table */}
<div className="tb"><table><thead><tr><th>{t.receipt}</th><th>{t.time}</th><th>👤 {rtl?"الكاشير":"Cashier"}</th><th>{t.items}</th><th>{t.method}</th><th>🏦 {rtl?"الحساب":"Account"}</th><th>{t.discount}</th><th>{t.vat}</th><th>{t.total}</th><th>{t.points}</th>{cu.role==="admin"&&<th>{t.act}</th>}</tr></thead>
<tbody>{sorted.length===0?<tr><td colSpan={cu.role==="admin"?11:10} style={{textAlign:"center",padding:40,color:"#9ca3af"}}>{t.noTxns}</td></tr>:sorted.map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}>
...
<td style={{fontFamily:"var(--m)"}}>{tx.items.reduce((s,i)=>s+i.qty,0)}</td>
<td><span style={{padding:"3px 10px",borderRadius:20,fontSize:10,fontWeight:600,background:tx.method==="cash"?"#ecfdf5":tx.method==="card"?"#eff6ff":"#fff7ed",color:tx.method==="cash"?"#059669":tx.method==="card"?"#2563eb":"#ea580c"}}>{tx.method==="mobile"?t.mada:tx.method==="card"?t.card:t.cash}</span></td>
...
<div className="tb"><div className="tbh"><span>{t.recent}</span><button className="ab ab-x" onClick={()=>exportXL(prods,txns,invs)}>📥 {t.excel}</button></div><table><thead><tr><th>{t.receipt}</th><th>{t.time}</th><th>👤</th><th>#</th><th>{t.method}</th><th>{t.total}</th></tr></thead><tbody>{!tC?<tr><td colSpan={6} style={{textAlign:"center",padding:30,color:"#9ca3af"}}>{t.noTxns}</td></tr>:txns.slice(0,15).map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}><td className="mn">{tx.rn}</td><td>{tx.date} {tx.time}</td><td style={{fontSize:11,color:tx.cashierName?"#1e40af":"#d1d5db"}}>{tx.cashierName||"—"}</td><td>{tx.items.reduce((s,i)=>s+i.qty,0)}</td><td>{tx.method==="mobile"?t.mada:tx.method==="card"?t.card:t.cash}</td><td className="mn" style={{color:"#059669"}}>{fm(tx.tot)}</td></tr>)}</tbody></table></div>
</div>}
...
{/* ── PRODUCTS SUB-TAB ── */}
{invSubTab==="products"&&<><div style={{display:"flex",gap:8,marginBottom:12}}><button className="ab ab-s" style={{padding:"8px 16px",fontSize:12}} onClick={()=>setAPM(true)}>{t.addProd}</button><button className="ab ab-x" style={{padding:"8px 16px",fontSize:12}} onClick={()=>exportXL(prods,txns,invs)}>{t.excel}</button></div>{prods.filter(p=>p.s<30).length>0&&<div className="lw"><div className="lwt">⚠️ {t.lowStock}</div>{prods.filter(p=>p.s<30).map(p=><div key={p.id} className="lwi">{pN(p)} — {p.s}</div>)}</div>}<div style={{overflowX:"auto"}}><table className="at"><thead><tr><th>{t.bc}</th><th>{t.product}</th><th>{t.cost}</th><th>{t.price}</th><th>{t.stock}</th><th>{rtl?"المباع":"Sold"}</th><th>{t.margin}</th><th>{t.expiryDate}</th><th>{rtl?"دُفعات":"Batches"}</th><th>{t.act}</th></tr></thead><tbody>{prods.map(p=>{const mg=p.p-p.c;const mgPct=p.c>0?((p.p-p.c)/p.c*100):0;const expDays=p.exp?Math.ceil((new Date(p.exp)-today2)/86400000):null;const pBatches=batches.filter(b=>b.product_id===p.id&&b.status==="active"&&b.quantity_remaining>0);return<tr key={p.id} style={{background:expDays!==null&&expDays<=0?"#fef2f2":expDays!==null&&expDays<=7?"#fffbeb":"transparent"}}><td style={{fontFamily:"var(--m)",fontSize:11}}>{p.bc}</td><td style={{fontWeight:600}}>{pN(p)}</td><td style={{fontFamily:"var(--m)"}}>{fN(p.c)}</td><td>{eProd===p.id?<input value={ePr} onChange={e=>setEPr(e.target.value)}/>:<span style={{fontFamily:"var(--m)",fontWeight:600}}>{fN(p.p)}</span>}</td><td>{eProd===p.id?<input value={eSt} onChange={e=>setESt(e.target.value)}/>:<span style={{fontWeight:600,color:p.s<=0?"#dc2626":p.s<10?"#d97706":"#059669"}}>{p.s}</span>}</td><td style={{fontFamily:"var(--m)",fontSize:11,textAlign:"center"}}>{(()=>{const sold=txns.reduce((s,tx)=>s+tx.items.filter(i=>i.id===p.id).reduce((a,i)=>a+i.qty,0),0);return<span style={{fontWeight:600,color:sold>0?"#2563eb":"#9ca3af"}}>{sold}</span>})()}</td><td style={{fontFamily:"var(--m)",fontSize:11}}><span style={{fontWeight:600,color:mg>0?"#059669":mg<0?"#dc2626":"#9ca3af"}}>{fN(mg)}</span><br/><span style={{fontSize:9,color:mgPct>=30?"#059669":mgPct>=15?"#d97706":"#dc2626"}}>{mgPct.toFixed(1)}%</span></td><td style={{fontSize:10}}>{eProd===p.id?<input type="date" value={eExp} onChange={e=>setEExp(e.target.value)} style={{width:120}}/>:p.exp?<span style={{fontFamily:"var(--m)",fontWeight:600,color:expDays<=0?"#dc2626":expDays<=7?"#ea580c":expDays<=30?"#d97706":"#059669"}}>{p.exp}{expDays<=0?" ⛔":expDays<=7?" 🔴":expDays<=30?" 🟡":""}</span>:<span style={{color:"#d1d5db"}}>—</span>}</td>
<td style={{minWidth:120}}>{pBatches.length>0?<div>
{pBatches.slice(0,3).map((b,bi)=>{const bExp=b.expiry_date?Math.ceil((new Date(b.expiry_date)-today2)/86400000):null;return<div key={bi} style={{fontSize:9,padding:"1px 0",color:bExp!==null&&bExp<=0?"#dc2626":bExp!==null&&bExp<=30?"#d97706":"#6b7280"}}>{b.quantity_remaining}× {b.expiry_date||"—"} {bExp!==null&&bExp<=0?"⛔":bExp!==null&&bExp<=7?"🔴":""}</div>})}
{pBatches.length>3&&<div style={{fontSize:8,color:"#9ca3af"}}>+{pBatches.length-3} {rtl?"أخرى":"more"}</div>}
<button onClick={()=>{setBatchMod(true);setNewBatch({product_id:p.id,batch_number:"B-"+Date.now().toString(36).toUpperCase(),supplier_name:"",received_date:new Date().toISOString().slice(0,10),expiry_date:"",quantity_received:"",cost_per_unit:p.c||"",notes:""})}} style={{fontSize:8,color:"#2563eb",background:"none",border:"none",cursor:"pointer",padding:0,marginTop:2}}>+ {rtl?"دُفعة":"batch"}</button>
</div>:<button onClick={()=>{setBatchMod(true);setNewBatch({product_id:p.id,batch_number:"B-"+Date.now().toString(36).toUpperCase(),supplier_name:"",received_date:new Date().toISOString().slice(0,10),expiry_date:"",quantity_received:"",cost_per_unit:p.c||"",notes:""})}} style={{fontSize:9,color:"#2563eb",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:6,padding:"2px 8px",cursor:"pointer",fontFamily:"var(--f)"}}>+ {rtl?"إضافة دُفعة":"Add Batch"}</button>}</td>
<td>{eProd===p.id?<><button className="ab ab-s" onClick={async()=>{const np=parseFloat(ePr)||p.p,ns=parseInt(eSt)||p.s,ne=eExp||p.exp||null;setProds(prev=>prev.map(x=>x.id===p.id?{...x,p:np,s:ns,exp:ne}:x));setEP(null);try{await DB.updateProductPriceStock(p.id,np,ns,ne)}catch(e){console.error(e)}}}>✓</button><button className="ab ab-c" onClick={()=>setEP(null)}>✕</button></>:<><button className="ab ab-e" onClick={()=>{setEP(p.id);setEPr(p.p.toString());setESt(p.s.toString());setEExp(p.exp||"")}}>✎ {t.edit}</button><button className="ab" style={{background:"#eff6ff",color:"#1e40af",border:"1px solid #bfdbfe",fontSize:10,padding:"4px 6px",marginLeft:2}} onClick={()=>{const win=window.open("","_blank","width=300,height=200");win.document.write(`<!DOCTYPE html><html><head><style>@page{size:50mm 30mm;margin:2mm}*{margin:0;font-family:Arial}body{display:flex;flex-direction:column;align-items:center;justify-content:center;height:26mm;text-align:center;direction:${rtl?"rtl":"ltr"}}.n{font-size:9pt;font-weight:bold;margin-bottom:2mm}.p{font-size:14pt;font-weight:900;color:#000}.bc{font-size:7pt;color:#666;margin-top:1mm;font-family:monospace}</style></head><body><div class="n">${(p.a||p.n).replace(/"/g,"")}</div><div class="p">${p.p.toFixed(3)} JD</div><div class="bc">${p.bc}</div></body></html>`);win.document.close();setTimeout(()=>{win.print();setTimeout(()=>win.close(),500)},200)}}>🏷️</button>{cu.role==="admin"&&p.s>0&&<button className="ab" style={{background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",fontSize:9,padding:"4px 8px",marginLeft:2}} onClick={()=>{const qty=prompt(rtl?"كم وحدة تريد إنقاصها؟":"How many units to decrease?","1");if(!qty)return;const n=parseInt(qty);if(isNaN(n)||n<=0){sT("✗ Invalid","err");return}if(n>p.s){sT("✗ "+(rtl?"أكثر من المخزون":"Exceeds stock"),"err");return}const reason=prompt(rtl?"السبب (تالف/منتهي/فقدان/تعديل)":"Reason (damaged/expired/loss/adjustment)","adjustment");const newStock=p.s-n;setProds(prev=>prev.map(x=>x.id===p.id?{...x,s:newStock}:x));DB.updateProductPriceStock(p.id,p.p,newStock,p.exp).catch(e=>console.error(e));sT("✓ "+(rtl?"تم إنقاص":"Decreased")+" "+n+" → "+newStock+" "+(reason||""),"ok")}}>▼ {rtl?"إنقاص":"Decrease"}</button>}<button className="ab ab-d" onClick={async()=>{if(!confirm(rtl?"حذف المنتج؟":"Delete product?"))return;setProds(prev=>prev.filter(x=>x.id!==p.id));try{await DB.deleteProduct(p.id)}catch(e){console.error(e)}}}>✕</button></>}</td></tr>})}</tbody></table></div></>}
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:b.status==="active"?"#ecfdf5":b.status==="depleted"?"#f3f4f6":"#fef2f2",color:b.status==="active"?"#059669":b.status==="depleted"?"#9ca3af":"#dc2626"}}>{b.status}</span></td>
...
{/* ── STOCKTAKE SUB-TAB ── */}
{invSubTab==="stocktake"&&<>
<h2 style={{fontSize:18}}>📊 {rtl?"الجرد ومطابقة المخزون":"Stocktake & Reconciliation"}</h2>

<div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
<button className="ab ab-s" style={{padding:"8px 16px",fontSize:13}} onClick={()=>{
  setStocktakeItems(prods.map(p=>({id:p.id,bc:p.bc,name:pN(p),systemQty:p.s,countedQty:"",diff:0})));
  sT("✓ "+(rtl?"تم تحميل المنتجات":"Products loaded — start counting"),"ok");
}}>📋 {rtl?"بدء جرد جديد":"Start New Count"}</button>
<button className="ab ab-x" style={{padding:"8px 16px",fontSize:13}} onClick={async()=>{
  const diffs=stocktakeItems.filter(i=>i.countedQty!==""&&parseInt(i.countedQty)!==i.systemQty);
  if(!diffs.length){sT(rtl?"لا فروقات":"No differences","ok");return}
  if(!confirm(rtl?"تعديل المخزون لـ "+diffs.length+" منتج؟":"Adjust stock for "+diffs.length+" items?"))return;
  for(const d of diffs){const newQty=parseInt(d.countedQty)||0;try{await DB.updateProductPriceStock(d.id,null,newQty);setProds(p=>p.map(x=>x.id===d.id?{...x,s:newQty}:x))}catch{}}
  sT("✓ "+(rtl?"تم تعديل "+diffs.length+" منتج":"Adjusted "+diffs.length+" items"),"ok");
}} disabled={!stocktakeItems.some(i=>i.countedQty!==""&&parseInt(i.countedQty)!==i.systemQty)}>✅ {rtl?"تطبيق الفروقات":"Apply Adjustments"}</button>
<button className="ab ab-x" style={{padding:"8px 16px",fontSize:13}} onClick={()=>{
  const diffs=stocktakeItems.filter(i=>i.countedQty!==""&&parseInt(i.countedQty)!==i.systemQty);
  const csvLines=["Product,Barcode,System Qty,Counted,Difference"];
  diffs.forEach(d=>csvLines.push(`"${d.name}","${d.bc}",${d.systemQty},${d.countedQty},${parseInt(d.countedQty)-d.systemQty}`));
  const blob=new Blob([csvLines.join("\n")],{type:"text/csv"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="stocktake-"+new Date().toISOString().slice(0,10)+".csv";a.click();
}}>📥 {rtl?"تصدير الفروقات":"Export Variances"}</button>
</div>

{/* Search */}
<div style={{marginBottom:12}}>
<input value={stocktakeSearch} onChange={e=>setStocktakeSearch(e.target.value)} placeholder={rtl?"بحث بالاسم أو الباركود...":"Search by name or barcode..."} style={{width:"100%",padding:"10px 16px",border:"1.5px solid #e2e8f0",borderRadius:10,fontSize:14,fontFamily:"var(--f)",outline:"none"}}/>
</div>

{/* Summary Cards */}
{stocktakeItems.length>0&&(()=>{
const counted=stocktakeItems.filter(i=>i.countedQty!=="");
const diffs=counted.filter(i=>parseInt(i.countedQty)!==i.systemQty);
const over=diffs.filter(i=>parseInt(i.countedQty)>i.systemQty);
const short=diffs.filter(i=>parseInt(i.countedQty)<i.systemQty);
return<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
<div style={{background:"#eff6ff",borderRadius:12,padding:14,textAlign:"center"}}><div style={{fontSize:11,color:"#1e40af",fontWeight:600}}>{rtl?"إجمالي الأصناف":"Total Items"}</div><div style={{fontSize:24,fontWeight:800,fontFamily:"var(--m)",color:"#1e40af"}}>{stocktakeItems.length}</div></div>
<div style={{background:"#ecfdf5",borderRadius:12,padding:14,textAlign:"center"}}><div style={{fontSize:11,color:"#059669",fontWeight:600}}>{rtl?"تم جردها":"Counted"}</div><div style={{fontSize:24,fontWeight:800,fontFamily:"var(--m)",color:"#059669"}}>{counted.length}</div></div>
<div style={{background:"#fef2f2",borderRadius:12,padding:14,textAlign:"center"}}><div style={{fontSize:11,color:"#dc2626",fontWeight:600}}>{rtl?"نقص":"Shortage"}</div><div style={{fontSize:24,fontWeight:800,fontFamily:"var(--m)",color:"#dc2626"}}>{short.length}</div></div>
<div style={{background:"#fffbeb",borderRadius:12,padding:14,textAlign:"center"}}><div style={{fontSize:11,color:"#d97706",fontWeight:600}}>{rtl?"زيادة":"Overage"}</div><div style={{fontSize:24,fontWeight:800,fontFamily:"var(--m)",color:"#d97706"}}>{over.length}</div></div>
</div>})()}

{/* Stocktake Table */}
{stocktakeItems.length===0?<div style={{textAlign:"center",padding:40,color:"#9ca3af"}}><div style={{fontSize:48,marginBottom:8}}>📊</div>{rtl?"اضغط 'بدء جرد جديد' لتحميل المنتجات":"Press 'Start New Count' to load products"}</div>:
<div style={{maxHeight:500,overflowY:"auto"}}><table className="at"><thead><tr style={{position:"sticky",top:0,background:"#fff",zIndex:1}}>
<th>{t.product}</th><th>{t.bc}</th><th style={{textAlign:"center"}}>{rtl?"النظام":"System"}</th><th style={{textAlign:"center",minWidth:80}}>{rtl?"الجرد":"Count"}</th><th style={{textAlign:"center"}}>{rtl?"الفرق":"Diff"}</th><th>{rtl?"الحالة":"Status"}</th>
</tr></thead><tbody>
{stocktakeItems.filter(i=>!stocktakeSearch||i.name.toLowerCase().includes(stocktakeSearch.toLowerCase())||i.bc.includes(stocktakeSearch)).map(item=>{
const diff=item.countedQty!==""?parseInt(item.countedQty)-item.systemQty:null;
return<tr key={item.id} style={{background:diff===null?"transparent":diff===0?"#f0fdf4":diff>0?"#fffbeb":"#fef2f2"}}>
<td style={{fontWeight:600,fontSize:13}}>{item.name}</td>
<td style={{fontFamily:"var(--m)",fontSize:11,color:"#6b7280"}}>{item.bc}</td>
<td style={{textAlign:"center",fontFamily:"var(--m)",fontWeight:600}}>{item.systemQty}</td>
<td style={{textAlign:"center"}}><input type="number" value={item.countedQty} onChange={e=>{setStocktakeItems(prev=>prev.map(x=>x.id===item.id?{...x,countedQty:e.target.value}:x))}} style={{width:70,textAlign:"center",padding:"6px",border:"1.5px solid "+(diff===null?"#e2e8f0":diff===0?"#86efac":diff>0?"#fde68a":"#fca5a5"),borderRadius:8,fontFamily:"var(--m)",fontSize:14,fontWeight:700,outline:"none",background:diff===null?"#fff":diff===0?"#ecfdf5":diff>0?"#fffbeb":"#fef2f2"}} placeholder="—"/></td>
<td style={{textAlign:"center",fontFamily:"var(--m)",fontWeight:700,fontSize:14,color:diff===null?"#d1d5db":diff===0?"#059669":diff>0?"#d97706":"#dc2626"}}>{diff===null?"—":diff>0?"+"+diff:diff}</td>
<td>{diff===null?<span style={{color:"#d1d5db",fontSize:11}}>⬜</span>:diff===0?<span style={{color:"#059669",fontSize:12}}>✅ {rtl?"مطابق":"Match"}</span>:diff>0?<span style={{color:"#d97706",fontSize:12}}>⚠️ {rtl?"زيادة":"Over"}</span>:<span style={{color:"#dc2626",fontSize:12}}>🔴 {rtl?"نقص":"Short"}</span>}</td>
</tr>})}
</tbody></table></div>}
</>}
...
<td><span style={{padding:"3px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:r.return_type==="full"?"#fef2f2":"#fffbeb",color:r.return_type==="full"?"#dc2626":"#d97706"}}>{r.return_type==="full"?(rtl?"كامل":"Full"):(rtl?"جزئي":"Partial")}</span></td>
...
<td style={{whiteSpace:"nowrap"}}><button className="ab ab-e" onClick={()=>{setEditUserMod(u);setEditUserData({un:u.un,fn:u.fn,fa:u.fa,role:u.role,perms:{...u.perms},avatar:u.avatar||null})}}>✎ {t.editUser}</button>{u.id!==1&&<button className="ab ab-d" style={{marginLeft:4}} onClick={async()=>{setUsers(p=>p.filter(x=>x.id!==u.id));try{await DB.deleteUser(u.id)}catch(e){console.error(e)}}}>✕</button>}</td>
</tr>)}</tbody></table>
...
<tbody>{txns.slice(0,50).map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}><td className="mn" style={{fontSize:11}}>{tx.rn}</td><td style={{fontSize:11}}>{tx.date} {tx.time}</td><td style={{fontSize:11,color:tx.cashierName?"#1e40af":"#d1d5db"}}>{tx.cashierName||"—"}</td><td style={{fontFamily:"var(--m)"}}>{tx.items.reduce((s,i)=>s+i.qty,0)}</td><td><span style={{padding:"2px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:tx.method==="cash"?"#ecfdf5":tx.method==="card"?"#eff6ff":"#fff7ed",color:tx.method==="cash"?"#059669":tx.method==="card"?"#2563eb":"#ea580c"}}>{tx.method==="mobile"?t.mada:tx.method==="card"?t.card:t.cash}</span></td><td className="mn" style={{color:"#059669"}}>{fm(tx.tot)}</td></tr>)}</tbody></table></div>
</>}
...
<span style={{padding:"2px 8px",borderRadius:14,fontSize:10,fontWeight:600,background:(typeColors[p.promo_type]||"#6b7280")+"18",color:typeColors[p.promo_type]||"#6b7280"}}>{typeLabels[p.promo_type]}</span>
</div>
...
{/* Custom Categories */}
<div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,padding:20,marginBottom:14}}>
<div style={{fontSize:14,fontWeight:700,color:"#374151",marginBottom:14}}>📁 {rtl?"إدارة الفئات":"Manage Categories"}</div>
<div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
{CATS.filter(c=>c.id!=="all").map(c=><span key={c.id} style={{padding:"4px 10px",borderRadius:8,fontSize:11,fontWeight:600,background:"#f3f4f6",color:"#374151"}}>{c.i} {t[c.k]||c.id}</span>)}
</div>
<div style={{display:"flex",gap:6}}>
<input id="new-cat-name" placeholder={rtl?"اسم الفئة الجديدة":"New category name"} style={{flex:1,padding:"8px 12px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:13,fontFamily:"var(--f)",outline:"none"}}/>
<input id="new-cat-emoji" placeholder="📁" style={{width:50,padding:"8px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:16,textAlign:"center",outline:"none"}}/>
<button onClick={()=>{const nm=document.getElementById("new-cat-name")?.value;const em=document.getElementById("new-cat-emoji")?.value||"📁";if(!nm)return;const id=nm.toLowerCase().replace(/\s+/g,"_").replace(/[^a-z0-9_أ-ي]/g,"");const nc=[...customCats,{id,name:nm,emoji:em}];setCustomCats(nc);localStorage.setItem("3045_custom_cats",JSON.stringify(nc));document.getElementById("new-cat-name").value="";document.getElementById("new-cat-emoji").value="";sT("✓ "+(rtl?"تمت إضافة فئة":"Category added")+": "+em+" "+nm,"ok")}} style={{padding:"8px 16px",background:"#059669",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>+</button>
</div>
...
{c.emoji} {c.name} <button onClick={()=>{const nc=customCats.filter((_,x)=>x!==i);setCustomCats(nc);localStorage.setItem("3045_custom_cats",JSON.stringify(nc))}} style={{background:"none",border:"none",color:"#dc2626",cursor:"pointer",fontSize:10,padding:0}}>✕</button>
</div>)}
</div>}
</div>
...
{["58mm","80mm","A4"].map(sz=><button key={sz} onClick={()=>{setPaperSize(sz);localStorage.setItem("3045_paper",sz)}} style={{padding:"6px 16px",borderRadius:8,border:paperSize===sz?"2px solid #1e40af":"1.5px solid #e2e8f0",background:paperSize===sz?"#eff6ff":"#fff",color:paperSize===sz?"#1e40af":"#64748b",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>{sz==="58mm"?"📜 58mm":sz==="80mm"?"🧾 80mm":"📄 A4"}</button>)}
</div>

{/* Receipt content */}
<div id="receipt-print-area" style={{fontFamily:"'Courier New',monospace",fontSize:paperSize==="A4"?13:paperSize==="80mm"?11:9,maxWidth:paperSize==="A4"?"100%":paperSize==="80mm"?300:220,margin:"0 auto",background:"#fff",padding:paperSize==="A4"?"30px":"12px",border:"1px dashed #d1d5db",borderRadius:8}}>

{/* Header */}
<div style={{textAlign:"center",marginBottom:paperSize==="A4"?16:8}}>
<img src={STORE_LOGO} alt="3045" style={{height:paperSize==="A4"?60:paperSize==="80mm"?40:30,objectFit:"contain",marginBottom:4}}/><div style={{fontSize:paperSize==="A4"?22:paperSize==="80mm"?16:13,fontWeight:900}}>{storeSettings.storeName||"3045 Supermarket"}</div>
...
rcMod.items.map((i,x)=><div key={x} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",fontSize:paperSize==="80mm"?11:9}}>
...
{/* Totals */}
<div style={{padding:"4px 0",fontSize:paperSize==="A4"?13:paperSize==="80mm"?11:9}}>
...
<div style={{borderTop:"2px solid #000",marginTop:4,paddingTop:4,display:"flex",justifyContent:"space-between",fontSize:paperSize==="A4"?18:paperSize==="80mm"?15:12,fontWeight:900}}>
<span>{t.total} (JD)</span><span>{fN(rcMod.tot)}</span>
</div>
</div>

{/* Payment info */}
<div style={{padding:"4px 0",fontSize:paperSize==="A4"?12:paperSize==="80mm"?10:8,color:"#6b7280",borderTop:"1px dashed #000",marginTop:4}}>
...
{/* Footer */}
<div style={{textAlign:"center",fontSize:paperSize==="A4"?12:paperSize==="80mm"?10:8,color:"#9ca3af",marginTop:6,paddingTop:6,borderTop:"1px dashed #000"}}>
...
if(!el)return;
const w=paperSize==="58mm"?"58mm":paperSize==="80mm"?"80mm":"210mm";
const h=paperSize==="A4"?"297mm":"auto";
const win=window.open("","_blank","width=400,height=600");
win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Receipt ${rcMod.rn}</title>
<style>
@page{size:${w} ${h};margin:${paperSize==="A4"?"10mm":"2mm"}}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Courier New',monospace;font-size:${paperSize==="A4"?"12pt":paperSize==="80mm"?"9pt":"7pt"};
width:${w};margin:0 auto;padding:${paperSize==="A4"?"10mm":"2mm"};direction:${rtl?"rtl":"ltr"}}
table{width:100%;border-collapse:collapse}
th,td{padding:${paperSize==="A4"?"4px":"2px"};text-align:${rtl?"right":"left"}}
th{border-bottom:1px solid #000;font-weight:bold}
.c{text-align:center}.r{text-align:${rtl?"left":"right"}}.b{font-weight:bold}
.line{border-top:1px dashed #000;margin:4px 0;padding-top:4px}
.total-line{border-top:2px solid #000;margin-top:4px;padding-top:4px;font-size:${paperSize==="A4"?"16pt":"12pt"};font-weight:900}
...
<div className="pf"><label>{t.nameEn}</label><div style={{display:"flex",gap:4}}><input value={nP.n} onChange={e=>setNP({...nP,n:e.target.value})} style={{flex:1}}/>{nP.a&&!nP.n&&<button onClick={()=>setNP(prev=>({...prev,n:prev.a}))} style={{padding:"4px 8px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:6,fontSize:10,cursor:"pointer",flexShrink:0}}>← AR</button>}</div></div><div className="pf"><label>{t.nameAr}</label><div style={{display:"flex",gap:4}}><input value={nP.a} onChange={e=>setNP({...nP,a:e.target.value})} style={{flex:1,direction:"rtl"}}/>{nP.n&&!nP.a&&<button onClick={()=>setNP(prev=>({...prev,a:prev.n}))} style={{padding:"4px 8px",background:"#ecfdf5",border:"1px solid #d1fae5",borderRadius:6,fontSize:10,cursor:"pointer",flexShrink:0}}>EN →</button>}</div></div><div style={{display:"flex",gap:8}}><div className="pf" style={{flex:1}}><label>{t.cost} (JD)</label><input type="number" value={nP.c} onChange={e=>setNP({...nP,c:e.target.value})}/></div><div className="pf" style={{flex:1}}><label>{t.price} (JD)</label><input type="number" value={nP.p} onChange={e=>setNP({...nP,p:e.target.value})}/></div></div><div style={{display:"flex",gap:8}}><div className="pf" style={{flex:1}}><label>{t.cat}</label><select value={nP.cat} onChange={e=>setNP({...nP,cat:e.target.value})}>{CATS.filter(c=>c.id!=="all").map(c=><option key={c.id} value={c.id}>{t[c.k]}</option>)}</select></div><div className="pf" style={{flex:1}}><label>{t.unit}</label><input value={nP.u} onChange={e=>setNP({...nP,u:e.target.value})}/></div></div><div className="pf"><label>Emoji</label><input value={nP.e} onChange={e=>setNP({...nP,e:e.target.value})}/></div>
...
<div style={{display:"flex",gap:8}}>
<div className="pf" style={{flex:1}}><label>{t.supplier}</label><input value={nP.supplier||""} onChange={e=>setNP({...nP,supplier:e.target.value})} placeholder={rtl?"اسم المورد":"Supplier name"}/></div>
...
<div className="pf" style={{flex:1}}><label>{rtl?"عدد القطع":"Pack size"}</label><input type="number" value={nP.packSize||""} onChange={e=>setNP({...nP,packSize:e.target.value})} placeholder="6"/></div>
</div>
...
const newProd={id:"S"+Date.now().toString(36),bc:nP.bc,n:nP.n,a:nP.a||nP.n,p:parseFloat(nP.p)||0,c:parseFloat(nP.c)||0,cat:nP.cat,u:nP.u,s:totalStock,e:nP.e,exp:nP.exp||null,img:nP.img||null};
...
try{
  ...
  if(nP.isPackage&&nP.parentBarcode&&nP.packSize){
    ...
    if(parentProd){
      // Store package info in localStorage for now
      const pkgs=JSON.parse(localStorage.getItem("3045_packages")||"{}");
      pkgs[newProd.bc]={parentId:parentProd.id,parentBc:nP.parentBarcode,packSize:parseInt(nP.packSize)};
      localStorage.setItem("3045_packages",JSON.stringify(pkgs));
    }
  }
}catch(e){console.error(e)}
setNP({bc:"",n:"",a:"",p:"",c:"",cat:"food",u:"pc",e:"📦",exp:"",img:null,supplier:"",initQty:"",batches:[],isPackage:false,parentBarcode:"",packSize:""});
...
{/* PURCHASE INVOICE */}
{invMod&&<div className="ov" onClick={()=>setInvMod(false)}><div className="md" onClick={e=>e.stopPropagation()} style={{maxWidth:520}}><h2>🧾 {t.addInv}<button className="mc" onClick={()=>setInvMod(false)}>✕</button></h2><div style={{display:"flex",gap:8}}><div className="pf" style={{flex:1}}><label>{t.supplier}</label><input value={invSup} onChange={e=>setInvSup(e.target.value)}/></div><div className="pf" style={{flex:1}}><label>{t.invNo}</label><input value={invNo} onChange={e=>setInvNo(e.target.value)}/></div></div><div style={{fontSize:13,fontWeight:700,margin:"10px 0 8px"}}>{t.invItems}:</div>{invItems.map((it,i)=><div key={i} className="inv-row" style={{position:"relative",marginTop:it.name?16:0}}><input value={it.barcode||""} onChange={e=>{const v=[...invItems];v[i]={...v[i],barcode:e.target.value};setInvItems(v)}} onKeyDown={async e=>{if(e.key!=="Enter"||!it.barcode)return;const bc=it.barcode.trim();const existing=prods.find(p=>p.bc===bc);if(existing){const v=[...invItems];v[i]={...v[i],prodId:existing.id,name:pN(existing),cost:existing.c.toString(),barcode:bc};setInvItems(v);sT("✓ "+pN(existing),"ok");return}try{const{data:catItem}=await sb.from("product_catalog").select("*").eq("barcode",bc).single();if(catItem){const newId="S"+Date.now().toString(36);const newP={id:newId,bc,n:catItem.name,a:catItem.name_ar,p:catItem.price||0,c:catItem.cost||0,cat:catItem.category||"food",u:"pc",s:0,e:catItem.emoji||"📦"};await DB.upsertProduct(newP);setProds(p=>[...p,newP]);const v=[...invItems];v[i]={...v[i],prodId:newId,name:catItem.name_ar||catItem.name,cost:catItem.cost?.toString()||"",barcode:bc};setInvItems(v);sT("✓ "+(rtl?"تم إنشاء منتج جديد":"New product created")+": "+(catItem.name_ar||catItem.name),"ok");return}}catch{}const nm=prompt(rtl?"المنتج غير موجود. أدخل اسم المنتج:":"Product not found. Enter product name:");if(nm){const newId="S"+Date.now().toString(36);const cp=prompt(rtl?"سعر البيع (JD):":"Sell price (JD):","0");const newP={id:newId,bc,n:nm,a:nm,p:parseFloat(cp)||0,c:0,cat:"food",u:"pc",s:0,e:"📦"};await DB.upsertProduct(newP);setProds(p=>[...p,newP]);const v=[...invItems];v[i]={...v[i],prodId:newId,name:nm,barcode:bc};setInvItems(v);sT("✓ "+(rtl?"تم إنشاء":"Created")+": "+nm,"ok")}}} placeholder={rtl?"امسح الباركود + Enter":"Scan barcode + Enter"} style={{flex:2,fontFamily:"var(--m)",fontSize:12}}/>{it.name&&<span style={{position:"absolute",top:-14,left:0,fontSize:9,color:"#059669",fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",maxWidth:200}}>{it.name}</span>}<input type="number" value={it.qty} onChange={e=>{const v=[...invItems];v[i]={...v[i],qty:e.target.value};setInvItems(v)}} placeholder={t.qty} style={{flex:1}}/><input type="number" value={it.cost} onChange={e=>{const v=[...invItems];v[i]={...v[i],cost:e.target.value};setInvItems(v)}} placeholder={t.costPr} style={{flex:1}}/>
<input type="date" value={it.expiry||""} onChange={e=>{const v=[...invItems];v[i]={...v[i],expiry:e.target.value};setInvItems(v)}} title={rtl?"تاريخ الانتهاء":"Expiry date"} style={{flex:1,fontSize:11}}/>
<button title={rtl?"تقسيم لدفعة أخرى بتاريخ مختلف":"Split for different expiry"} onClick={()=>{setInvItems(p=>{const n=[...p];n.splice(i+1,0,{...p[i],qty:"",expiry:"",_split:true});return n})}} style={{background:"none",border:"1.5px solid #86efac",borderRadius:6,color:"#059669",cursor:"pointer",fontSize:10,padding:"4px 6px",flexShrink:0,fontFamily:"var(--f)",fontWeight:700}}>+📅</button>
{invItems.length>1&&<button className="inv-rm" onClick={()=>setInvItems(p=>p.filter((_,x)=>x!==i))}>✕</button>}</div>)}<button onClick={()=>setInvItems(p=>[...p,{prodId:"",qty:"",cost:"",barcode:"",name:"",expiry:""}])} style={{background:"none",border:"2px dashed #d1d5db",borderRadius:10,color:"#6b7280",padding:"8px",width:"100%",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"var(--f)",marginBottom:12}}>{t.addItem}</button>
...
{/* VIEW INVOICE */}
{invView&&<div className="ov" onClick={()=>setInvView(null)}><div className="md" onClick={e=>e.stopPropagation()} style={{maxWidth:640}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
<h2 style={{margin:0}}>🧾 {invView.invoiceNo}</h2>
<div style={{display:"flex",gap:6}}>
<button onClick={()=>{
const d=rtl?"rtl":"ltr";
const items=invView.items;
const total=invView.totalCost;
const win=window.open("","_blank","width=800,height=600");
win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Invoice ${invView.invoiceNo}</title>
<style>@page{size:A4;margin:15mm}*{margin:0;padding:0;box-sizing:border-box}
body{font-family:Arial,sans-serif;font-size:11pt;direction:${d};padding:20mm}
.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;padding-bottom:15px;border-bottom:3px double #1e3a5f}
.logo{font-size:24pt;font-weight:900;color:#1e3a5f}
.logo-sub{font-size:9pt;color:#666}
.inv-title{font-size:18pt;font-weight:bold;color:#1e3a5f;text-align:center;margin:15px 0}
.info-grid{display:flex;justify-content:space-between;margin-bottom:20px}
.info-box{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px;min-width:45%}
.info-label{font-size:9pt;color:#64748b;margin-bottom:3px}
.info-val{font-size:11pt;font-weight:bold;color:#1e293b}
table{width:100%;border-collapse:collapse;margin:15px 0}
th{background:#1e3a5f;color:white;padding:10px 8px;text-align:${d==="rtl"?"right":"left"};font-size:10pt}
td{padding:8px;border-bottom:1px solid #e2e8f0;font-size:10pt}
tr:nth-child(even){background:#f8fafc}
.total-row{background:#ecfdf5 !important;font-weight:bold;font-size:12pt}
.total-row td{border-top:2px solid #059669;padding:12px 8px}
.footer{margin-top:30px;border-top:2px solid #e2e8f0;padding-top:15px;display:flex;justify-content:space-between}
.sign-box{text-align:center;min-width:200px}
.sign-line{border-top:1px solid #000;margin-top:40px;padding-top:5px;font-size:9pt;color:#666}
.r{text-align:${d==="rtl"?"left":"right"}}
@media print{body{padding:10mm}}
</style></head><body>
<div class="header">
<div><div class="logo">3045</div><div class="logo-sub">SUPERMARKET</div>
<div class="logo-sub">${rtl?"إربد، شارع المدينة المنورة - مقابل SOS":"Irbid, Almadina Almonawarah St. (Opp. SOS)"}</div>
<div class="logo-sub">📞 0791191244</div></div>
<div style="text-align:${d==="rtl"?"left":"right"}">
<div class="inv-title">${rtl?"فاتورة مشتريات":"PURCHASE INVOICE"}</div>
<div style="font-size:10pt;color:#64748b">${rtl?"رقم الفاتورة":"Invoice No"}: <strong>${invView.invoiceNo}</strong></div>
<div style="font-size:10pt;color:#64748b">${rtl?"التاريخ":"Date"}: <strong>${invView.date}</strong></div>
</div></div>
<div class="info-grid">
<div class="info-box"><div class="info-label">${rtl?"المورد":"Supplier"}</div><div class="info-val">🏭 ${invView.supplier}</div></div>
<div class="info-box"><div class="info-label">${rtl?"استلمها":"Received By"}</div><div class="info-val">👤 ${invView.receivedBy||"—"}</div></div>
</div>
<table><thead><tr><th>#</th><th>${rtl?"المنتج":"Product"}</th><th class="r">${rtl?"الكمية":"Qty"}</th><th class="r">${rtl?"سعر الوحدة":"Unit Cost"}</th><th class="r">${rtl?"الإجمالي":"Total"}</th></tr></thead>
<tbody>${items.map((it,i)=>`<tr><td>${i+1}</td><td style="font-weight:600">${it.productName||it.name||"—"}</td><td class="r">${it.qty}</td><td class="r">${(parseFloat(it.cost)||0).toFixed(3)}</td><td class="r" style="font-weight:bold">${((parseFloat(it.cost)||0)*(parseInt(it.qty)||0)).toFixed(3)}</td></tr>`).join("")}
<tr class="total-row"><td colspan="4" style="text-align:${d==="rtl"?"left":"right"};font-size:14pt">${rtl?"الإجمالي":"TOTAL"}</td><td class="r" style="font-size:14pt;color:#059669">${total.toFixed(3)} JD</td></tr>
</tbody></table>
${invView.payMethod?`<div style="margin-top:10px;font-size:10pt;color:#64748b">${rtl?"طريقة الدفع":"Payment"}: <strong>${invView.payMethod==="cash"?"💵 Cash":invView.payMethod==="bank"?"🏦 Bank":"📄 Credit"}</strong>${invView.debitAcct?" ← "+invView.debitAcct:""}</div>`:""}
<div class="footer">
<div class="sign-box"><div class="sign-line">${rtl?"توقيع المستلم":"Receiver Signature"}</div></div>
<div class="sign-box"><div class="sign-line">${rtl?"توقيع المدير":"Manager Signature"}</div></div>
</div>
<div style="text-align:center;margin-top:20px;font-size:8pt;color:#9ca3af">${rtl?"طُبع بواسطة نظام 3045 سوبرماركت":"Printed by 3045 Supermarket POS System"} · ${new Date().toLocaleString()}</div>
</body></html>`);
win.document.close();setTimeout(()=>{win.print();setTimeout(()=>win.close(),500)},300);
}} style={{padding:"8px 16px",background:"#1e40af",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>🖨 {rtl?"طباعة":"Print"}</button>
<button className="mc" onClick={()=>setInvView(null)}>✕</button>
</div>
</div>

{/* Invoice Preview */}
<div id="inv-preview" style={{background:"#fff",border:"1.5px solid #e2e8f0",borderRadius:16,padding:24}}>

{/* Header */}
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingBottom:16,borderBottom:"3px double #1e3a5f",marginBottom:16}}>
<div>
<img src={STORE_LOGO} alt="3045" style={{height:50,objectFit:"contain",marginBottom:4}}/>
<div style={{fontSize:10,color:"#6b7280"}}>{rtl?"إربد، شارع المدينة المنورة - مقابل SOS":"Irbid, Almadina Almonawarah St. (Opp. SOS)"}</div>
<div style={{fontSize:10,color:"#6b7280"}}>📞 0791191244</div>
</div>
<div style={{textAlign:rtl?"left":"right"}}>
<div style={{fontSize:18,fontWeight:800,color:"#1e3a5f"}}>{rtl?"فاتورة مشتريات":"PURCHASE INVOICE"}</div>
<div style={{fontSize:12,color:"#6b7280",marginTop:4}}>{rtl?"رقم":"No"}: <strong style={{color:"#1e3a5f"}}>{invView.invoiceNo}</strong></div>
<div style={{fontSize:12,color:"#6b7280"}}>{rtl?"التاريخ":"Date"}: <strong>{invView.date}</strong></div>
</div>
</div>
...
<div style={{fontSize:10,color:"#64748b",marginBottom:4}}>{t.supplier}</div>
<div style={{fontSize:14,fontWeight:700,color:"#1e293b"}}>🏭 {invView.supplier}</div>
</div>
<div style={{flex:1,background:"#f8fafc",borderRadius:10,padding:12,border:"1px solid #e2e8f0"}}>
<div style={{fontSize:10,color:"#64748b",marginBottom:4}}>{t.by}</div>
<div style={{fontSize:14,fontWeight:700,color:"#1e293b"}}>👤 {invView.receivedBy||"—"}</div>
</div>
</div>
...
<th style={{padding:"10px 8px",textAlign:"center",borderRadius:rtl?"0 8px 0 0":"8px 0 0 0"}}>#</th>
<th style={{padding:"10px 8px"}}>{t.product}</th>
<th style={{padding:"10px 8px",textAlign:"center"}}>{t.qty}</th>
<th style={{padding:"10px 8px",textAlign:"center"}}>{t.costPr}</th>
<th style={{padding:"10px 8px",textAlign:rtl?"left":"right",borderRadius:rtl?"8px 0 0 0":"0 8px 0 0"}}>{t.total}</th>
</tr></thead>
<tbody>{invView.items.map((it,i)=><tr key={i} style={{background:i%2===0?"#fff":"#f8fafc",borderBottom:"1px solid #e2e8f0"}}>
<td style={{padding:"8px",textAlign:"center",color:"#9ca3af"}}>{i+1}</td>
<td style={{padding:"8px",fontWeight:600}}>{it.productName||it.name||"—"}</td>
<td style={{padding:"8px",textAlign:"center",fontFamily:"var(--m)"}}>{it.qty}</td>
<td style={{padding:"8px",textAlign:"center",fontFamily:"var(--m)"}}>{fN(parseFloat(it.cost)||0)}</td>
<td style={{padding:"8px",textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:700,color:"#059669"}}>{fN((parseFloat(it.cost)||0)*(parseInt(it.qty)||0))}</td>
</tr>)}
<tr style={{background:"#ecfdf5",borderTop:"2px solid #059669"}}>
<td colSpan={4} style={{padding:"12px 8px",textAlign:rtl?"left":"right",fontWeight:800,fontSize:16}}>{t.totCost}</td>
<td style={{padding:"12px 8px",textAlign:rtl?"left":"right",fontFamily:"var(--m)",fontWeight:900,fontSize:18,color:"#059669"}}>{fm(invView.totalCost)}</td>
</tr>
</tbody></table>

{/* Payment Info */}
{invView.payMethod&&<div style={{marginTop:12,padding:"8px 12px",background:"#f8fafc",borderRadius:8,fontSize:12,color:"#6b7280"}}>
{rtl?"طريقة الدفع":"Payment"}: <strong>{invView.payMethod==="cash"?"💵 "+(rtl?"نقدي":"Cash"):invView.payMethod==="bank"?"🏦 "+(rtl?"تحويل بنكي":"Bank Transfer"):"📄 "+(rtl?"آجل":"Credit")}</strong>
{invView.debitAcct&&<span> ← {invView.debitAcct}</span>}
</div>}
</div>
...
<div style={{display:"flex",alignItems:"center",gap:8}}>
<button onClick={()=>document.getElementById("inv-attach-later")?.click()} style={{flex:1,padding:"10px 16px",background:"#fffbeb",border:"1.5px dashed #fcd34d",borderRadius:10,color:"#92400e",fontSize:12,cursor:"pointer",fontFamily:"var(--f)"}}>📎 {rtl?"إرفاق صورة الفاتورة":"Attach invoice image"}</button>
</div>
...
{/* Profile fields */}
<div className="pf"><label>🔢 {rtl?"الرقم الوظيفي / اسم المستخدم":"Employee ID / Username"}</label><input value={editUserData.un||editUserMod.un} onChange={e=>setEditUserData({...editUserData,un:e.target.value})} placeholder={rtl?"مثال: 1001":"e.g. 1001"} style={{fontFamily:"var(--m)",direction:"ltr"}}/></div>
<div style={{display:"flex",gap:8}}>
...
<button className="cpb" onClick={async()=>{
  const newUn=editUserData.un||editUserMod.un;
  const updates={username:newUn,full_name:editUserData.fn,full_name_ar:editUserData.fa,role:editUserData.role,permissions:editUserData.perms,avatar:editUserData.avatar};
  setUsers(p=>p.map(u=>u.id===editUserMod.id?{...u,un:newUn,fn:editUserData.fn,fa:editUserData.fa,role:editUserData.role,perms:editUserData.perms,avatar:editUserData.avatar}:u));
  if(cu.id===editUserMod.id)setCU(prev=>({...prev,un:newUn,fn:editUserData.fn,fa:editUserData.fa,role:editUserData.role,perms:editUserData.perms,avatar:editUserData.avatar}));
