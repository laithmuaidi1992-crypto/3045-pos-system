...
<button onClick={()=>{const newBc="2"+Date.now().toString().slice(-11);setNP({...nP,bc:newBc});sT("✓ "+(rtl?"تم توليد باركود":"Barcode generated"),"ok")}} title={rtl?"توليد باركود جديد للمنتجات بدون باركود":"Auto-generate for items without barcode"} style={{padding:"10px 14px",background:"#7c3aed",border:"none",borderRadius:"var(--r)",color:"#fff",fontSize:12,cursor:"pointer",flexShrink:0,fontFamily:"var(--f)",fontWeight:700}}>🎲 {rtl?"توليد":"Auto"}</button>
