...
const auditGaps = (() => {
  ...
  recentTxs.forEach(t => {
    ...
    if(!byCashierDay[dayKey]) byCashierDay[dayKey] = {cashier:t.cashierName, date:new Date(t.ts).toISOString().slice(0,10), seqs:[]};
    if(t.seq) byCashierDay[dayKey].seqs.push(t.seq);
  });
  const gaps = [];
  Object.values(byCashierDay).forEach(g => {
    ...
    for(let i=1; i<sorted.length; i++) {
      if(sorted[i]-sorted[i-1] > 1) {
        for(let m=sorted[i-1]+1; m<sorted[i]; m++) {
          gaps.push({cashier: g.cashier, date: g.date, missing: m});
        }
      }
    }
  });
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div>
        ...
        <div style={{fontSize:11,color:"#7f1d1d",marginTop:4}}>
          {rtl?`اكتُشف ${auditGaps.length} فاتورة مفقودة — قد تكون حُذفت يدوياً أو فُقدت`:`Detected ${auditGaps.length} missing receipts — may have been manually deleted or lost`}
        </div>
      </div>
      <button onClick={()=>{setTab("admin");setAT("reconcile")}}
        style={{padding:"8px 16px",background:"#dc2626",border:"none",borderRadius:8,color:"#fff",fontWeight:700,cursor:"pointer",fontSize:12,whiteSpace:"nowrap"}}>
        💰 {rtl?"عرض في المطابقة":"View in Reconciliation"}
      </button>
    </div>
    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}}>
      {auditGaps.slice(0,15).map((g,i)=>(
        <span key={i} style={{padding:"3px 10px",background:"#dc2626",color:"#fff",borderRadius:6,fontSize:10,fontFamily:"monospace",fontWeight:700}}>
          {g.cashier} · {g.date} · #{g.missing}
        </span>
      ))}
      {auditGaps.length>15 && <span style={{fontSize:11,color:"#7f1d1d",fontWeight:600,padding:"3px 10px"}}>+{auditGaps.length-15} {rtl?"أخرى":"more"}</span>}
    </div>
  </div>
)}
