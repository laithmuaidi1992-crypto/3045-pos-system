...
const targetDaily=500;const pct=Math.min(100,(todayTotal/targetDaily*100));

return<div className="dsh" style={{background:"linear-gradient(180deg,#f0f9ff 0%,#f9fafb 30%)"}}>

{/* Hero Banner */}
<div style={{background:"linear-gradient(135deg,#1e3a5f 0%,#2563eb 50%,#7c3aed 100%)",borderRadius:24,padding:"28px 28px 24px",color:"#fff",position:"relative",overflow:"hidden"}}>
...
<div style={{fontSize:14,opacity:.8,marginBottom:4}}>{greeting} 👋</div>
<div style={{fontSize:26,fontWeight:800,marginBottom:2}}>{rtl?(cu.fa||cu.fn):cu.fn}</div>
<div style={{fontSize:12,opacity:.7}}>{cu.role==="admin"?t.adminR:cu.role==="manager"?t.manager:t.cashier} · {new Date().toLocaleDateString(rtl?"ar":"en-US",{weekday:"long",month:"long",day:"numeric"})}</div>

{/* Clock In/Out inline */}
<div style={{marginTop:16,display:"flex",alignItems:"center",gap:12}}>
{myAtt&&myAtt.clock_in&&!myAtt.clock_out?<>
<div style={{background:"rgba(255,255,255,.15)",borderRadius:12,padding:"8px 16px",fontSize:12}}>🕐 {t.clockedIn} {new Date(myAtt.clock_in).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}</div>
<button onClick={async()=>{try{const a=await DB.clockOut(cu.id);setMyAtt(a);sT("✓ "+t.clockedOut,"ok")}catch{}}} style={{padding:"10px 24px",background:"#dc2626",border:"none",borderRadius:12,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)",boxShadow:"0 4px 12px rgba(220,38,38,.4)"}}>🕐 {t.clockOut}</button>
</>:myAtt&&myAtt.clock_out?<>
<div style={{background:"rgba(255,255,255,.15)",borderRadius:12,padding:"8px 16px",fontSize:12}}>✓ {new Date(myAtt.clock_in).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})} → {new Date(myAtt.clock_out).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})} · <strong>{myAtt.hours_worked}h</strong></div>
</>:<>
<button onClick={async()=>{try{const a=await DB.clockIn(cu.id);setMyAtt(a);sT("✓ "+t.clockedIn,"ok")}catch{}}} style={{padding:"12px 32px",background:"rgba(255,255,255,.2)",border:"2px solid rgba(255,255,255,.4)",borderRadius:12,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)",backdropFilter:"blur(4px)"}}>🕐 {t.clockIn}</button>
</>}
</div>
</div></div>
...
{/* Recent Transactions */}
<div className="tb"><div className="tbh"><span>🕐 {t.recentActivity}</span></div><table><thead><tr><th>{t.receipt}</th><th>{t.time}</th><th>👤</th><th>{t.method}</th><th>{t.total}</th></tr></thead><tbody>{txns.slice(0,5).map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}><td className="mn" style={{fontSize:11}}>{tx.rn}</td><td style={{fontSize:11}}>{tx.date} {tx.time}</td><td style={{fontSize:11,color:tx.custName?"#2563eb":"#d1d5db"}}>{tx.custName||"—"}</td><td><span style={{padding:"2px 8px",borderRadius:14,fontSize:9,fontWeight:600,background:tx.method==="cash"?"#ecfdf5":tx.method==="card"?"#eff6ff":"#f5f3ff",color:tx.method==="cash"?"#059669":tx.method==="card"?"#2563eb":"#7c3aed"}}>{tx.method==="mobile"?t.mada:tx.method==="card"?t.card:t.cash}</span></td><td className="mn" style={{color:"#059669"}}>{fm(tx.tot)}</td></tr>)}</tbody></table></div>
</div>})()}
