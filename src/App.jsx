...
.mn{flex-direction:column}.pp{min-height:50vh}.cp{width:100%;border-left:none;border-top:1px solid var(--g200);max-height:45vh}
...
return<div key={i} style={{background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,padding:16,position:"relative"}}>
...
<div style={{display:"flex",alignItems:"center",gap:10}}>
<div style={{fontSize:28}}>{typeIcons[doc.type]||"📄"}</div>
<div>
...
{cu.role==="admin"&&<button className="ab ab-d" style={{fontSize:9}} onClick={()=>{if(!confirm(rtl?"حذف؟":"Delete?"))return;saveDocuments(documents.filter((_,x)=>x!==i))}}>✕</button>}
</div>
{doc.description&&<div style={{fontSize:11,color:"#6b7280",marginTop:8}}>{doc.description}</div>}
<div style={{fontSize:10,color:"#9ca3af",marginTop:6,fontFamily:"var(--m)"}}>{doc.date}</div>
{doc.file&&<button onClick={()=>setViewDocMod(doc)} style={{marginTop:8,padding:"6px 14px",background:"#eff6ff",border:"1.5px solid #bfdbfe",borderRadius:8,color:"#2563eb",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"var(--f)",width:"100%"}}>👁 {t.viewDoc} — {doc.fileName}</button>}
</div>})}
</div>}
</>}
...
{/* SALE TAB */}
{tab==="sale"&&<><div className="pp"><div className="sb"><div className="sw"><span className="sw-icon">🔍</span><input className="si" placeholder={t.search} value={search} onChange={e=>setSearch(e.target.value)}/></div><button className="bb" onClick={()=>setBM(true)}>▦ {t.barcode}</button><button className="bb" onClick={()=>{setBM(true);setCamScan(true)}} style={{background:"#eff6ff",borderColor:"#bfdbfe",color:"#2563eb"}}>📷</button></div><div className="cats">{CATS.map(c=><button key={c.id} className={"ch "+(cat===c.id?"a":"")} onClick={()=>setCat(c.id)}>{c.i} {t[c.k]}</button>)}</div><div className="pg">{fp.map(p=><div key={p.id} className="pc" onClick={()=>addToCart(p)}>{p.s<30&&<div className="pl"/>}{p.img?<img src={p.img} style={{width:40,height:40,objectFit:"cover",borderRadius:8}} alt=""/>:<div className="pe">{p.e}</div>}<div className="pn">{pN(p)}</div><div className="pm"><span className="pp2">{fN(p.p)}</span><span className="pu">/ {p.u}</span></div></div>)}{!fp.length&&<div style={{gridColumn:"1/-1",textAlign:"center",padding:40,color:"#9ca3af"}}>{t.none}</div>}</div></div>
...
{atab==="purchases"&&<><h2>🧾 {t.purchases}</h2><button className="ab ab-s" style={{padding:"8px 16px",fontSize:12,marginBottom:12}} onClick={()=>setInvMod(true)}>{t.addInv}</button>{!invs.length?<div style={{textAlign:"center",padding:40,color:"#9ca3af"}}>📋 {t.noInv}</div>:invs.map(inv=><div key={inv.id} className="inv-card" onClick={()=>setInvView(inv)}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontFamily:"var(--m)",fontSize:13,fontWeight:700,color:"#2563eb"}}>{inv.invoiceNo}</span><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:"#9ca3af"}}>{inv.date}</span>{cu.role==="admin"&&<button className="ab ab-d" style={{fontSize:9,padding:"2px 6px"}} onClick={async(e)=>{e.stopPropagation();if(!confirm(rtl?"حذف الفاتورة؟":"Delete invoice?"))return;setInvs(p=>p.filter(x=>x.id!==inv.id));try{await DB.deleteInvoice(inv.id)}catch{}}}>✕</button>}</div></div><div style={{fontSize:13,fontWeight:500}}>🏭 {inv.supplier}</div><div style={{fontSize:12,color:"#9ca3af",marginTop:4}}>{inv.items.length} {t.items} · <span style={{color:"#059669",fontFamily:"var(--m)",fontWeight:700}}>{fm(inv.totalCost)}</span></div></div>)}</>}
...
<button className="cpb" style={{background:"#dc2626"}} onClick={async()=>{if(!newExp.category_id||!newExp.amount)return;const amt=parseFloat(newExp.amount)||0;const e={category_id:newExp.category_id,amount:amt,description:newExp.description,payment_method:newExp.payment_method,expense_date:newExp.expense_date,recurring:newExp.recurring,reference_no:newExp.reference_no,attachment:newExp.attachment||null,created_by:cu?.id};try{const r=await DB.addExpense(e);if(r)setExpensesList(p=>[r,...p]);
// Auto-debit from bank account
if(newExp.debit_account&&amt>0){const acct=bankAccts.find(a=>a.id===+newExp.debit_account);if(acct){const newBal=+acct.balance-amt;await DB.updateBankBalance(acct.id,newBal);await DB.addMoneyMovement({account_id:acct.id,type:"withdrawal",amount:amt,balance_after:newBal,description:(newExp.description||t.expenses)+" — "+newExp.reference_no,reference_no:newExp.reference_no,created_by:cu?.id});setBankAccts(p=>p.map(a=>a.id===acct.id?{...a,balance:newBal}:a));const mv=await DB.getMoneyMovements();setMovements(mv)}}
setExpMod(false);sT("✓ "+t.saved,"ok")}catch(er){console.error(er)}}} disabled={!newExp.category_id||!newExp.amount}>✓ {t.addExpense} — {fm(parseFloat(newExp.amount)||0)}</button>
</div></div>}
...
{/* VIEW DOCUMENT MODAL */}
{viewDocMod&&<div className="ov" onClick={()=>setViewDocMod(null)}><div className="md" onClick={e=>e.stopPropagation()} style={{maxWidth:600}}>
<h2>📄 {viewDocMod.title}<button className="mc" onClick={()=>setViewDocMod(null)}>✕</button></h2>
<div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
<span style={{padding:"4px 12px",borderRadius:20,fontSize:11,fontWeight:600,background:"#eff6ff",color:"#2563eb"}}>{t[viewDocMod.type==="rent"?"rentContract":viewDocMod.type==="license"?"license":viewDocMod.type==="insurance"?"insurance":viewDocMod.type==="agreement"?"agreement":"otherDoc"]}</span>
<span style={{fontSize:11,color:"#9ca3af",fontFamily:"var(--m)"}}>{viewDocMod.date}</span>
</div>
{viewDocMod.description&&<div style={{fontSize:13,color:"#6b7280",marginBottom:12}}>{viewDocMod.description}</div>}
{viewDocMod.file&&(viewDocMod.file.startsWith("data:image")?
<img src={viewDocMod.file} style={{width:"100%",borderRadius:12,border:"1px solid #e5e7eb"}} alt=""/>:
