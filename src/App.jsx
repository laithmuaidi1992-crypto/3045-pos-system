...
// Barcode lookup from Open Food Facts (free, no API key)
const lookupBarcode=async(code)=>{
  ...
  try{
    ...
    if(data.status===1&&data.product){
      ...
      let cat="snacks";
      if(cats.some(c=>c.includes("beverage")||c.includes("drink")))cat="beverages";
      else if(cats.some(c=>c.includes("chocolate")||c.includes("candy")||c.includes("sweet")))cat="candy";
      else if(cats.some(c=>c.includes("chip")||c.includes("snack")||c.includes("crisp")))cat="snacks";
      else if(cats.some(c=>c.includes("water")||c.includes("juice")))cat="water";
      else if(cats.some(c=>c.includes("can")||c.includes("sauce")||c.includes("paste")))cat="canned";
      else if(cats.some(c=>c.includes("care")||c.includes("hygiene")||c.includes("tooth")||c.includes("soap")))cat="personal";
      else if(cats.some(c=>c.includes("clean")||c.includes("house")))cat="household";
      // Try to get image
      let img=null;
      ...
      setNP(prev=>({...prev,n:fullName||prev.n,a:nameAr||prev.a,cat,e:p.ecoscore_grade?"📦":"📦",img:img||prev.img}));
      sT("✓ "+(rtl?"تم العثور على المنتج":"Product found")+": "+fullName,"ok");
    }else{sT("⚠ "+(rtl?"المنتج غير موجود في قاعدة البيانات العالمية":"Product not found in global database — enter manually"),"err")}
...
const[storeSettings,setStoreSettings]=useState(()=>{try{return JSON.parse(localStorage.getItem("3045_settings"))||{storeName:"3045 Super Grocery Shopping",taxRate:0,currency:"JD",dailyTarget:500,weeklyTarget:3000,monthlyTarget:12000}}catch{return{storeName:"3045 Super Grocery Shopping",taxRate:0,currency:"JD",dailyTarget:500,weeklyTarget:3000,monthlyTarget:12000}}});
...
const addToCart=useCallback(p=>{beep();setCart(prev=>{const ex=prev.find(i=>i.id===p.id);return ex?prev.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i):[...prev,{...p,qty:1}]})},[beep]);
...
<div style={{fontSize:18,fontWeight:800,color:"#1f2937"}}>3045 Super Grocery Shopping</div>
...
<div style={{fontSize:20,fontWeight:800,fontFamily:"var(--f)"}}>{storeSettings.storeName||"3045 Super Grocery Shopping"}</div><div style={{fontSize:10,color:"#6b7280"}}>{rtl?"إربد، شارع المدينة المنورة":"Irbid, Almadina Almonawarah St."} · 📞 0791191244</div>
...
{/* ═══ SALE TAB — REDESIGNED ═══ */}
{tab==="sale"&&<div style={{display:"flex",flex:1,overflow:"hidden",height:"100%"}}>
...
{CATS.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:"14px 6px",background:cat===c.id?"#1e40af":"#fff",border:cat===c.id?"none":"1.5px solid #e2e8f0",borderRadius:10,fontSize:11,fontWeight:700,color:cat===c.id?"#fff":"#475569",cursor:"pointer",fontFamily:"var(--f)",textAlign:"center",lineHeight:1.3,transition:"all .15s"}}>{c.i}<br/>{rtl?t[c.k]:t[c.k]}</button>)}
</div>
...
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:12,fontWeight:600,color:"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pN(p)}</div>
<div style={{fontSize:9,color:"#94a3b8",fontFamily:"var(--m)"}}>{p.bc}</div>
</div>
<div style={{fontFamily:"var(--m)",fontSize:13,fontWeight:800,color:"#1e40af",flexShrink:0}}>{fN(p.p)}</div>
</div>)}
...
{/* Summary grid */}
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px 20px",fontSize:12,marginBottom:8}}>
...
<span style={{fontSize:28,fontWeight:900,color:"#dc2626",fontFamily:"var(--m)"}}>{fm(selCust&&redeemPts>0?totAfterRedeem:tot)}</span>
</div>

{/* Action buttons row */}
<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
<button onClick={()=>{setPM("cash");setCT("")}} disabled={!cart.length} style={{flex:"1 1 80px",padding:"12px 8px",background:"#059669",border:"none",borderRadius:10,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",opacity:cart.length?"1":".3",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
<span style={{fontSize:16}}>💵</span>{t.cash}
</button>
<button onClick={()=>{setPM("card");setCT("")}} disabled={!cart.length} style={{flex:"1 1 80px",padding:"12px 8px",background:"#1e40af",border:"none",borderRadius:10,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",opacity:cart.length?"1":".3",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
<span style={{fontSize:16}}>💳</span>{t.card}
</button>
<button onClick={()=>{if(!cart.length)return;setHeld(p=>[...p,{id:gI(),items:[...cart],time:new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"}),date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric"}),disc:aDisc}]);clr()}} disabled={!cart.length} style={{flex:"1 1 80px",padding:"12px 8px",background:"#7c3aed",border:"none",borderRadius:10,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",opacity:cart.length?"1":".3",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
...
<button onClick={()=>{setPM("mobile");setCT("")}} disabled={!cart.length} style={{flex:"1 1 80px",padding:"12px 8px",background:"#0d9488",border:"none",borderRadius:10,color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"var(--f)",opacity:cart.length?"1":".3",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
...
{/* Stock summary */}
<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
<div style={{background:"#ecfdf5",borderRadius:12,padding:12,textAlign:"center"}}><div style={{fontSize:9,color:"#059669",fontWeight:600}}>{rtl?"إجمالي المنتجات":"Total Products"}</div><div style={{fontSize:22,fontWeight:800,fontFamily:"var(--m)",color:"#059669"}}>{prods.length}</div></div>
<div style={{background:"#eff6ff",borderRadius:12,padding:12,textAlign:"center"}}><div style={{fontSize:9,color:"#1e40af",fontWeight:600}}>{rtl?"متوفر":"In Stock"}</div><div style={{fontSize:22,fontWeight:800,fontFamily:"var(--m)",color:"#1e40af"}}>{prods.filter(p=>p.s>0).length}</div></div>
<div style={{background:"#fffbeb",borderRadius:12,padding:12,textAlign:"center"}}><div style={{fontSize:9,color:"#d97706",fontWeight:600}}>{rtl?"مخزون منخفض":"Low Stock"}</div><div style={{fontSize:22,fontWeight:800,fontFamily:"var(--m)",color:"#d97706"}}>{prods.filter(p=>p.s>0&&p.s<10).length}</div></div>
<div style={{background:"#fef2f2",borderRadius:12,padding:12,textAlign:"center"}}><div style={{fontSize:9,color:"#dc2626",fontWeight:600}}>{rtl?"نفذ":"Out of Stock"}</div><div style={{fontSize:22,fontWeight:800,fontFamily:"var(--m)",color:"#dc2626"}}>{prods.filter(p=>p.s<=0).length}</div></div>
</div>
...
try{await DB.closeShift(activeShift.id,u);setCashShifts(p=>p.map(s=>s.id===activeShift.id?{...s,...u}:s));setActiveShift(null);setShiftCashCount("");sT("✓ "+(rtl?"تم إغلاق الوردية":"Shift closed")+(diff>0.01?" ↑ +"+fm(diff):diff<-0.01?" ↓ "+fm(diff):" ✓ Match"),"ok")}catch(e){console.error(e)}
...
{/* Table */}
<div className="tb"><table><thead><tr><th>{t.receipt}</th><th>{t.time}</th><th>👤 {t.customers}</th><th>{t.items}</th><th>{t.method}</th><th>🏦 {rtl?"الحساب":"Account"}</th><th>{t.discount}</th><th>{t.vat}</th><th>{t.total}</th><th>{t.points}</th>{cu.role==="admin"&&<th>{t.act}</th>}</tr></thead>
...
<div style={{fontSize:paperSize==="A4"?22:paperSize==="80mm"?16:13,fontWeight:900}}>{storeSettings.storeName||"3045 Super Grocery"}</div>
<div style={{fontSize:paperSize==="A4"?12:9,color:"#6b7280"}}>{rtl?"إربد، شارع المدينة المنورة":"Irbid, Almadina Almonawarah St."}</div>
...
{storeSettings.storeName||"3045 Super Grocery Shopping"} · {new Date().getFullYear()}
</div>
</div>
...
<input value={nP.bc} onChange={e=>setNP({...nP,bc:e.target.value})} onKeyDown={e=>{if(e.key==="Enter"&&nP.bc.length>=8)lookupBarcode(nP.bc)}} style={{flex:1}} placeholder={rtl?"امسح أو اكتب الباركود":"Scan or type barcode"}/>
<button onClick={()=>lookupBarcode(nP.bc)} disabled={!nP.bc||nP.bc.length<8||bcLookup} style={{padding:"10px 16px",background:bcLookup?"#d1d5db":"#059669",border:"none",borderRadius:"var(--r)",color:"#fff",fontSize:12,cursor:"pointer",flexShrink:0,fontFamily:"var(--f)",fontWeight:700,opacity:(!nP.bc||nP.bc.length<8)?".4":"1"}}>{bcLookup?"⏳":"🔍"} {rtl?"بحث":"Lookup"}</button>
...
<div style={{fontSize:18,fontWeight:800,fontFamily:"var(--f)"}}>{storeSettings.storeName||"3045 Super Grocery Shopping"}</div><div style={{fontSize:10,color:"#6b7280"}}>{rtl?"إربد، شارع المدينة المنورة":"Irbid, Almadina Almonawarah St."} · 📞 0791191244</div>
