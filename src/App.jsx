  ...
  async function load(){
    try{
      ...
      try{
        ...
        // Load gap audit reset timestamp (stored as individual setting)
        if(allSettings.gap_audit_start_date){
          setStoreSettings(prev => ({...prev, gap_audit_start_date: allSettings.gap_audit_start_date}));
        }
...
const tTgross=txns.reduce((s,t2)=>s+t2.tot,0),tC=txns.length,tIS=txns.reduce((s,t2)=>s+t2.items.reduce((a,i)=>a+i.qty,0),0);
const totalReturns=(salesReturns||[]).reduce((s,r)=>s+(+r.total_refund||0),0);
const tT=tTgross-totalReturns; // Net total sales
const aT=tC>0?tT/tC:0;
...
// Sales (gross)
const todaySalesGross=txns.filter(tx=>{try{return new Date(tx.ts).toDateString()===todayStr}catch{return false}}).reduce((s,t2)=>s+t2.tot,0);
const weekSalesGross=txns.filter(tx=>{try{return new Date(tx.ts)>=weekAgo}catch{return false}}).reduce((s,t2)=>s+t2.tot,0);
const monthSalesGross=txns.filter(tx=>{try{return new Date(tx.ts)>=monthAgo}catch{return false}}).reduce((s,t2)=>s+t2.tot,0);
// Returns (refunds)
const todayReturns=(salesReturns||[]).filter(r=>{try{return new Date(r.created_at).toDateString()===todayStr}catch{return false}}).reduce((s,r)=>s+(+r.total_refund||0),0);
const weekReturns=(salesReturns||[]).filter(r=>{try{return new Date(r.created_at)>=weekAgo}catch{return false}}).reduce((s,r)=>s+(+r.total_refund||0),0);
const monthReturns=(salesReturns||[]).filter(r=>{try{return new Date(r.created_at)>=monthAgo}catch{return false}}).reduce((s,r)=>s+(+r.total_refund||0),0);
// Net sales = gross - returns
const todaySales=todaySalesGross-todayReturns;
const weekSales=weekSalesGross-weekReturns;
const monthSales=monthSalesGross-monthReturns;
...
// Profit (subtract returned goods profit too)
const todayProfitGross=txns.filter(tx=>{try{return new Date(tx.ts).toDateString()===todayStr}catch{return false}}).reduce((s,tx)=>s+tx.items.reduce((a,i)=>{const pr=prods.find(p=>p.id===i.id);return a+(i.p-(pr?pr.c:0))*i.qty},0),0);
// Subtract returned items' profit from profit calc
const todayReturnedProfit=(salesReturns||[]).filter(r=>{try{return new Date(r.created_at).toDateString()===todayStr}catch{return false}}).reduce((s,r)=>s+(+r.total_refund||0)*0.3,0); // approx profit on refunds (conservative 30%)
const todayProfit=todayProfitGross-todayReturnedProfit;
...
const getTxnReturnStatus = (tx) => {
  if(!tx || !tx.rn || tx.rn === "" || tx.rn === null) return {status:"none", totalReturned:0, refundAmount:0};
  // Only consider returns with a valid receipt_no matching tx.rn exactly (no empty strings)
  const myReturns = salesReturns.filter(r => r && r.receipt_no && r.receipt_no === tx.rn);
  if(myReturns.length === 0) return {status:"none", totalReturned:0, refundAmount:0};
  const totalRefund = myReturns.reduce((s,r) => s + (+r.total_refund || 0), 0);
  ...
  // Require tx.tot > 0 to prevent false-match on 0-value transactions
  if(isFullReturn || (tx.tot > 0 && Math.abs(totalRefund - tx.tot) < 0.01)) {
...
<div className="dc" style={{borderLeft:"4px solid #059669"}}>
  <div className="dcl">📅 {t.today}</div>
  <div className="dcv g">{fm(todaySales)}</div>
  <div className="dcc">
    {todayTxns} {t.txns.toLowerCase()} · {todayItems} {t.items.toLowerCase()}
    {todayReturns > 0 && <div style={{fontSize:10,color:"#dc2626",fontWeight:700,marginTop:2}}>↩️ {rtl?"مرتجع":"Returned"}: −{fm(todayReturns)}</div>}
  </div>
</div>
<div className="dc" style={{borderLeft:"4px solid #2563eb"}}>
  <div className="dcl">📆 {t.week}</div>
  <div className="dcv b">{fm(weekSales)}</div>
  <div className="dcc">
    {tIS} {t.sold}
    {weekReturns > 0 && <div style={{fontSize:10,color:"#dc2626",fontWeight:700,marginTop:2}}>↩️ −{fm(weekReturns)}</div>}
  </div>
</div>
<div className="dc" style={{borderLeft:"4px solid #7c3aed"}}>
  <div className="dcl">📅 {t.month}</div>
  <div className="dcv p">{fm(monthSales)}</div>
  {monthReturns > 0 && <div className="dcc"><div style={{fontSize:10,color:"#dc2626",fontWeight:700}}>↩️ −{fm(monthReturns)}</div></div>}
</div>
...
const auditGaps = (() => {
  ...
  // Check for gap audit reset timestamp (ignores older gaps)
  const gapResetIso = storeSettings.gap_audit_start_date || null;
  const gapResetDate = gapResetIso ? new Date(gapResetIso) : null;
  // Use the LATEST of: 7 days ago, or the reset timestamp
  const cutoff = gapResetDate && gapResetDate > sevenDaysAgo ? gapResetDate : sevenDaysAgo;
  // Include VOIDED transactions too (they keep their seq)
  const recentTxs = txns.filter(t => {try{return new Date(t.ts) >= cutoff}catch{return false}});
  const byCashierDay = {};
  recentTxs.forEach(t => {
...
{auditGaps.length === 0 && storeSettings.gap_audit_start_date && (
  <div style={{padding:12,background:"linear-gradient(135deg,#ecfdf5,#d1fae5)",border:"2px solid #10b981",borderRadius:12,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
    <div>
      <div style={{fontSize:13,fontWeight:800,color:"#065f46"}}>✅ {rtl?"لا توجد فجوات في الترقيم":"No Receipt Number Gaps"}</div>
      <div style={{fontSize:10,color:"#047857",marginTop:3}}>
        {rtl?"نقطة البداية: ":"Audit started: "}<span style={{fontFamily:"monospace",fontWeight:700}}>{new Date(storeSettings.gap_audit_start_date).toLocaleString()}</span>
      </div>
    </div>
    {cu.role === "admin" && (
      <button onClick={async()=>{
        if(!confirm(rtl?"إلغاء نقطة البداية؟ سيتم فحص آخر 7 أيام كاملة.":"Remove audit start date? Will check full last 7 days.")) return;
        try{
          await DB.setSetting("gap_audit_start_date", "");
          setStoreSettings(prev => ({...prev, gap_audit_start_date: ""}));
          sT("✓ "+(rtl?"تم الإلغاء":"Removed"),"ok");
        }catch(e){sT("✗ "+e.message,"err")}
      }} style={{padding:"6px 12px",background:"#fff",color:"#dc2626",border:"1.5px solid #fca5a5",borderRadius:6,fontSize:10,fontWeight:700,cursor:"pointer"}}>
        🗑 {rtl?"إلغاء التصفير":"Undo Reset"}
      </button>
    )}
  </div>
)}
  ...
  <div style={{padding:14,background:"linear-gradient(135deg,#fef2f2,#fee2e2)",border:"2px solid #dc2626",borderRadius:12,marginBottom:14}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
      <div style={{flex:1,minWidth:250}}>
        <div style={{fontSize:14,fontWeight:800,color:"#991b1b"}}>🔴 {rtl?"تنبيه: فجوات في أرقام الفواتير":"ALERT: Receipt Number Gaps"}</div>
        ...
        {storeSettings.gap_audit_start_date && (
          <div style={{fontSize:10,color:"#059669",marginTop:4,fontWeight:600}}>
            ✓ {rtl?"نقطة البداية: ":"Audit started: "}<span style={{fontFamily:"monospace"}}>{new Date(storeSettings.gap_audit_start_date).toLocaleString()}</span>
          </div>
        )}
      </div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        ...
        {cu.role === "admin" && (
          <button onClick={async()=>{
            if(!confirm(rtl?
              "🧹 تصفير الفجوات\n\nسيتم تجاهل كل الفجوات السابقة. من الآن فصاعداً، سيبدأ الفحص من اللحظة الحالية فقط.\n\nالفجوات القديمة ستبقى مسجلة في سجل التدقيق للمراجعة لاحقاً.\n\nمتابعة؟":
              "🧹 Reset Gap Audit\n\nAll previous gaps will be ignored. From now on, audit will start from current moment only.\n\nOld gaps remain in audit log for later review.\n\nContinue?"
            )) return;
            try{
              const now = new Date().toISOString();
              await DB.setSetting("gap_audit_start_date", now);
              setStoreSettings(prev => ({...prev, gap_audit_start_date: now}));
              // Audit log
              await DB.addAuditLog({
                user_id: cu.id, user_name: cu.fn,
                action: "gap_audit_reset",
                entity_type: "system", entity_id: 0,
                field_name: "gap_audit_start_date",
                old_value: "",
                new_value: now,
                notes: `Gap audit reset by ${cu.fn}. Ignored ${auditGaps.length} historical gaps.`
              }).catch(()=>{});
              sT("✓ "+(rtl?"تم تصفير الفجوات بنجاح":"Gap audit reset successfully"),"ok");
            }catch(e){
              console.error(e);
              sT("✗ "+e.message,"err");
            }
          }} style={{padding:"8px 14px",background:"linear-gradient(135deg,#059669,#10b981)",border:"none",borderRadius:8,color:"#fff",fontWeight:800,cursor:"pointer",fontSize:11,whiteSpace:"nowrap",boxShadow:"0 2px 6px rgba(5,150,105,.3)"}}>
            🧹 {rtl?"ابدأ من الآن":"Start Fresh"}
          </button>
        )}
