...
{/* RECEIPT */}
{rcMod&&<div className="ov" onClick={()=>setRM(null)}><div className="md" onClick={e=>e.stopPropagation()}><div className="rcpt"><div className="rh"><h2>3045 Super Grocery</h2><p>Jordan · Tax# 123456789</p><p>{rcMod.date} · {rcMod.time} · {rcMod.rn}</p>{rcMod.custName&&<p style={{marginTop:4,fontWeight:600,color:"#1e40af"}}>👤 {rcMod.custName} · {rcMod.custPhone}</p>}</div>{rcMod.items.map((i,x)=><div key={x} className="ri"><span className="rin">{pN(i)}</span><span className="riq">×{i.qty}</span><span className="rit">{fN(i.p*i.qty)}</span></div>)}<hr className="rd"/><div className="rsr"><span>{t.subtotal}</span><span style={{fontFamily:"var(--m)",fontWeight:600}}>{fN(rcMod.sub)}</span></div>{rcMod.dp>0&&<div className="rsr"><span>{t.discount} ({rcMod.dp}%)</span><span style={{fontFamily:"var(--m)"}}>−{fN(rcMod.disc)}</span></div>}<div className="rsr"><span>{t.vat}</span><span style={{fontFamily:"var(--m)"}}>{fN(rcMod.tax)}</span></div>{rcMod.ptsRedeemed>0&&<div className="rsr" style={{color:"#7c3aed"}}><span>🎁 {t.redeemPts} ({rcMod.ptsRedeemed})</span><span style={{fontFamily:"var(--m)"}}>−{fN(DB.pointsToJD(rcMod.ptsRedeemed))}</span></div>}<hr className="rd"/><div className="rsr T"><span>{t.total} (JD)</span><span style={{fontFamily:"var(--m)"}}>{fN(rcMod.tot)}</span></div>
{/* Payment method + bank account */}
<div className="rsr" style={{fontSize:11,color:"#6b7280",marginTop:4}}>
<span>{t.method}</span>
<span style={{fontWeight:600,color:rcMod.method==="cash"?"#059669":rcMod.method==="card"?"#2563eb":"#7c3aed"}}>{rcMod.method==="cash"?"💵 "+t.cash:rcMod.method==="card"?"💳 "+t.card:"📱 "+t.mada}</span>
</div>
{(()=>{const acctName=rcMod.method==="cash"?(rtl?"صندوق النقد":"Cash Register"):(rtl?"الحساب البنكي الرئيسي":"Main Bank Account");return<div className="rsr" style={{fontSize:10,color:"#9ca3af"}}><span>🏦 {rtl?"مودع في":"Deposited to"}</span><span>{acctName}</span></div>})()}
{rcMod.method==="cash"&&rcMod.ct>0&&<><div className="rsr" style={{fontSize:11,color:"#6b7280"}}><span>{t.tendered}</span><span style={{fontFamily:"var(--m)"}}>{fN(rcMod.ct)}</span></div><div className="rsr" style={{fontSize:11,color:"#059669",fontWeight:600}}><span>{t.change}</span><span style={{fontFamily:"var(--m)"}}>{fN(rcMod.ch)}</span></div></>}{rcMod.ptsEarned>0&&<div style={{textAlign:"center",margin:"8px 0",padding:6,background:"#ecfdf5",borderRadius:8,fontSize:11,color:"#059669",fontWeight:600}}>⭐ +{rcMod.ptsEarned} {t.points} {t.earned}</div>}<div className="rf">Thank you for shopping at 3045!<br/>شكراً لتسوقكم في 3045!</div></div>
