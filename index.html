...
const DB = {
  ...
  // Users
  async getUsers() { const {data}=await sb.from("pos_users").select("*").order("id"); return (data||[]).map(r=>({id:r.id,un:r.username,fn:r.full_name,fa:r.full_name_ar||r.full_name,role:r.role,st:r.status,pw:r.password,perms:r.permissions||{pos:true,dashboard:false,inventory:false,purchases:false,sales_view:false,users:false,loyalty:false,settings:false,excel_export:false}})); },
...
const hL=()=>{const f=users.find(u=>u.un===lu&&u.pw===lp&&u.st==="active");if(f){setCU(f);setLI(true);setLE(false);setTab("home");DB.getTodayAttendance(f.id).then(a=>setMyAtt(a)).catch(()=>{})}else setLE(true)};
...
{/* HOME PAGE */}
{tab==="home"&&<div className="dsh">
<div style={{textAlign:"center",padding:"20px 0 10px"}}>
<div style={{fontSize:40,marginBottom:8}}>🏪</div>
<h1 style={{fontSize:24,fontWeight:800,color:"#1f2937",margin:0}}>{t.welcome2}</h1>
<p style={{color:"#6b7280",fontSize:13,marginTop:4}}>{new Date().toLocaleDateString(rtl?"ar":"en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}</p>
</div>

{/* Clock In/Out */}
<div style={{background:"linear-gradient(135deg,#eff6ff,#f0fdf4)",border:"1.5px solid #bfdbfe",borderRadius:20,padding:20,textAlign:"center",marginBottom:6}}>
<div style={{fontSize:13,fontWeight:600,color:"#374151",marginBottom:8}}>👤 {rtl?(cu.fa||cu.fn):cu.fn} — {cu.role==="admin"?t.adminR:cu.role==="manager"?t.manager:t.cashier}</div>
{myAtt&&myAtt.clock_in&&!myAtt.clock_out?<>
<div style={{fontSize:11,color:"#059669",marginBottom:8}}>✓ {t.clockedIn}: {new Date(myAtt.clock_in).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}</div>
<button onClick={async()=>{try{const a=await DB.clockOut(cu.id);setMyAtt(a);sT("✓ "+t.clockedOut,"ok")}catch(e){console.error(e)}}} style={{padding:"12px 40px",background:"#dc2626",border:"none",borderRadius:12,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>🕐 {t.clockOut}</button>
</>:myAtt&&myAtt.clock_out?<>
<div style={{fontSize:11,color:"#6b7280"}}>✓ {t.clockedIn}: {new Date(myAtt.clock_in).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})} → {t.clockedOut}: {new Date(myAtt.clock_out).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}</div>
<div style={{fontSize:18,fontWeight:800,color:"#2563eb",fontFamily:"var(--m)",marginTop:4}}>{myAtt.hours_worked}h {t.hoursWorked}</div>
</>:<>
<button onClick={async()=>{try{const a=await DB.clockIn(cu.id);setMyAtt(a);sT("✓ "+t.clockedIn,"ok")}catch(e){console.error(e)}}} style={{padding:"14px 50px",background:"#059669",border:"none",borderRadius:12,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"var(--f)"}}>🕐 {t.clockIn}</button>
</>}
</div>

{/* Quick Actions */}
<div style={{marginBottom:6}}>
<div style={{fontSize:13,fontWeight:700,color:"#374151",marginBottom:10}}>{t.quickActions}</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8}}>
<button onClick={()=>setTab("sale")} style={{padding:16,background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,cursor:"pointer",fontFamily:"var(--f)",textAlign:"center",transition:"all .15s"}}><div style={{fontSize:28}}>🛒</div><div style={{fontSize:12,fontWeight:600,color:"#374151",marginTop:4}}>{t.newSale}</div></button>
{hasP("dashboard")&&<button onClick={()=>setTab("dashboard")} style={{padding:16,background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,cursor:"pointer",fontFamily:"var(--f)",textAlign:"center"}}><div style={{fontSize:28}}>📊</div><div style={{fontSize:12,fontWeight:600,color:"#374151",marginTop:4}}>{t.dashboard}</div></button>}
{hasP("inventory")&&<button onClick={()=>{setTab("admin");setAT("inventory")}} style={{padding:16,background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,cursor:"pointer",fontFamily:"var(--f)",textAlign:"center"}}><div style={{fontSize:28}}>📦</div><div style={{fontSize:12,fontWeight:600,color:"#374151",marginTop:4}}>{t.inventory}</div></button>}
{hasP("sales_view")&&<button onClick={()=>setTab("sales")} style={{padding:16,background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,cursor:"pointer",fontFamily:"var(--f)",textAlign:"center"}}><div style={{fontSize:28}}>📋</div><div style={{fontSize:12,fontWeight:600,color:"#374151",marginTop:4}}>{t.salesView}</div></button>}
{hasP("hr")&&<button onClick={()=>setTab("hr")} style={{padding:16,background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,cursor:"pointer",fontFamily:"var(--f)",textAlign:"center"}}><div style={{fontSize:28}}>🏢</div><div style={{fontSize:12,fontWeight:600,color:"#374151",marginTop:4}}>{t.hr}</div></button>}
{hasP("excel_export")&&<button onClick={()=>exportXL(prods,txns,invs)} style={{padding:16,background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:16,cursor:"pointer",fontFamily:"var(--f)",textAlign:"center"}}><div style={{fontSize:28}}>📥</div><div style={{fontSize:12,fontWeight:600,color:"#374151",marginTop:4}}>{t.excel}</div></button>}
</div></div>

{/* System Overview */}
<div style={{fontSize:13,fontWeight:700,color:"#374151",marginBottom:10}}>{t.systemOverview}</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
<div className="dc" style={{borderLeft:"4px solid #059669"}}><div className="dcl">{t.today}</div><div className="dcv g">{fm(txns.filter(tx=>{try{return new Date(tx.ts).toDateString()===new Date().toDateString()}catch{return false}}).reduce((s,t2)=>s+t2.tot,0))}</div></div>
<div className="dc" style={{borderLeft:"4px solid #2563eb"}}><div className="dcl">{t.txns}</div><div className="dcv b">{tC}</div></div>
<div className="dc" style={{borderLeft:"4px solid #7c3aed"}}><div className="dcl">{t.customers}</div><div className="dcv p">{customers.length}</div></div>
<div className="dc" style={{borderLeft:"4px solid #d97706"}}><div className="dcl">{t.inventory}</div><div className="dcv y">{prods.length} / {prods.reduce((s,p)=>s+p.s,0)}</div></div>
</div>

{/* Recent Activity */}
<div className="tb"><div className="tbh"><span>{t.recentActivity}</span></div><table><thead><tr><th>{t.receipt}</th><th>{t.time}</th><th>👤</th><th>{t.total}</th></tr></thead><tbody>{txns.slice(0,5).map(tx=><tr key={tx.id} style={{cursor:"pointer"}} onClick={()=>setRM(tx)}><td className="mn" style={{fontSize:11}}>{tx.rn}</td><td style={{fontSize:11}}>{tx.date} {tx.time}</td><td style={{fontSize:11,color:tx.custName?"#2563eb":"#d1d5db"}}>{tx.custName||"—"}</td><td className="mn" style={{color:"#059669"}}>{fm(tx.tot)}</td></tr>)}</tbody></table></div>
</div>}
...
{s.status==="pending"&&<button className="ab ab-s" style={{marginLeft:4,fontSize:9}} onClick={async()=>{setSalPayments(p=>p.map(x=>x.id===s.id?{...x,status:"paid",payment_date:new Date().toISOString().slice(0,10)}:x));try{await DB.updateSalary(s.id,{status:"paid",payment_date:new Date().toISOString().slice(0,10)})}catch{}sT("✓ "+t.paid,"ok")}}>{t.markPaid}</button>}</td>
</tr>})}</tbody></table>}
</>}
...
const totalSalPaid=salPayments.filter(s=>s.status==="paid").reduce((s,p)=>s+ +p.net_salary,0);
const grossRev=tT;
...
const netP=grossP-totalExp-totalSalPaid;
...
<tr><td style={{fontWeight:600}}>💸 {t.opExpenses}</td><td className="mn" style={{color:"#dc2626"}}>-{fm(totalExp)}</td></tr>
...
{/* P&L STATEMENT */}
{finTab==="pnl"&&<><h2>📈 {t.pnl}</h2>
<div style={{background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:20,padding:24,maxWidth:500}}>
...
<div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #f3f4f6",color:"#dc2626"}}><span style={{fontWeight:500}}>💸 {t.opExpenses}</span><span style={{fontFamily:"var(--m)"}}>({fm(totalExp)})</span></div>
...
<td style={{fontWeight:500}}>{rtl?(u.fa||u.fn):u.fn}</td>
...
<td style={{whiteSpace:"nowrap"}}><button className="ab ab-e" onClick={()=>{setEditUserMod(u);setEditUserData({fn:u.fn,fa:u.fa,role:u.role,perms:{...u.perms}})}}>✎ {t.editUser}</button>{u.id!==1&&<button className="ab ab-d" style={{marginLeft:4}} onClick={async()=>{setUsers(p=>p.filter(x=>x.id!==u.id));try{await DB.deleteUser(u.id)}catch(e){console.error(e)}}}>✕</button>}</td>
</tr>)}</tbody></table>
...
<button className="cpb" onClick={async()=>{
  const updates={full_name:editUserData.fn,full_name_ar:editUserData.fa,role:editUserData.role,permissions:editUserData.perms};
  setUsers(p=>p.map(u=>u.id===editUserMod.id?{...u,fn:editUserData.fn,fa:editUserData.fa,role:editUserData.role,perms:editUserData.perms}:u));
